...
type WorkflowResult = {
  ...
  recipients?: Array<{ name: string; email: string; company?: string | null }>;
  retentionDays?: number;
...
export default function DropPublicTransferClient({ mode, slug }: Props) {
  ...
  return <main className="min-h-screen overflow-x-hidden bg-[#f4f9fa] px-4 py-8 sm:px-6"><div className="mx-auto max-w-5xl"><Header mode={mode}/>
    ...
    {sessionReady ? <section className="mt-8 rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
      ...
      {mode === "send" && sendMode === "quick_image" ? <section data-drop-quick-send-recipient-step className="mb-6 rounded-2xl border border-cyan-200 bg-cyan-50/50 p-4">
        <div className="mb-3"><p className="text-xs font-black uppercase tracking-[.12em] text-cyan-800">1. Címzettek</p><h3 className="mt-1 text-lg font-black text-slate-950">Ellenőrizze, kik kapják meg a küldeményt</h3><p className="mt-1 text-xs leading-5 text-slate-600">Ez a Gyors KépSend első lépése. A képek után a rendszer még egyszer visszakéri ezeket az adatokat ellenőrzésre.</p></div>
        ...
        <div className="mt-4"><Field label="Megőrzési idő"><select value={retentionDays} onChange={(event) => setRetentionDays(Number(event.target.value))} className={inputClass}>{[5, 7, 14, 30].map((day) => <option key={day} value={day}>{day} nap{day === defaultRetention ? " · alapérték" : ""}</option>)}</select></Field></div>
      </section> : null}
