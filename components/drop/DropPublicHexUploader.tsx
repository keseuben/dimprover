...
export default function DropPublicHexUploader({
  ...
  const [fileNameRule, setFileNameRule] = useState<DropFileNameRule>(imageOnly ? "dimpro_photo" : "safe_original");
  ...
  return <section data-drop-offline-queue className="relative mt-6 rounded-[1.75rem] border border-cyan-200 bg-white p-4 shadow-sm sm:p-6">
    ...
    {allowImageGroups ? <div className="mt-4 flex flex-col gap-2 rounded-2xl border border-teal-300 bg-white p-3 shadow-sm sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.1em] text-teal-800">Kamera / Galéria célcsoport</p><p className="mt-0.5 text-sm font-black text-slate-950">{selectedGroup?.name || "Csoport nélkül"}</p></div><select value={selectedGroupId} onChange={(event) => { setSelectedGroupId(event.target.value); setVisibleGroupId(event.target.value || "__ungrouped__"); }} disabled={running || preparing || creatingGroup} className="h-11 min-w-52 rounded-xl border border-teal-300 bg-teal-50 px-3 text-sm font-bold text-teal-950 outline-none focus:ring-4 focus:ring-teal-100"><option value="">Csoport nélkül</option>{groups.map((group) => <option key={group.id} value={group.id}>{group.name} · {groupDisplayCount(group.id)} fájl</option>)}</select></div> : null}
      ...
      <div className="mt-4 flex justify-end"><button type="button" onClick={() => goToStep(1)} className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-cyan-800 px-5 py-3 text-sm font-black text-white">Tovább a {imageOnly ? "képekhez" : "fájlokhoz"} <ArrowRight size={17}/></button></div>
    </div> : null}

    <div ref={imagesStepRef} className="scroll-mt-28"/>
    <div className={workflowStep === 1 ? "block" : "hidden"}>
      <div className="mt-4 rounded-2xl border border-cyan-100 bg-cyan-50/60 p-4"><p className="text-xs font-black uppercase tracking-[.12em] text-cyan-800">2. {imageOnly ? "Képek" : "Fájlok"}</p><h3 className="mt-1 text-lg font-black text-slate-950">{imageOnly ? "Adja hozzá a küldendő képeket" : "Adja hozzá a küldendő dokumentumokat és fájlokat"}</h3><p className="mt-1 text-sm leading-6 text-slate-600">{imageOnly ? "Használhatja a kamerát, a galériát vagy a fájlválasztót. A képeket még a következő lépésben ellenőrizheti." : "PDF, Office, CAD/BIM, ZIP, kép és más engedélyezett projektfájl is kiválasztható. A fájlokat a következő lépésben ellenőrizheti."}</p></div>
    <div ref={uploadZoneRef} className="mt-5"><DropHexUploadZone accept={imageOnly ? acceptedImageExtensions : acceptedExtensions} disabled={running || finalizing || delivered} busy={preparing || running || finalizing || restoring} imageMode={imageOnly} allowCamera={imageOnly} title={imageOnly ? "Fotók hozzáadása" : "Dokumentumok és fájlok hozzáadása"} description={imageOnly ? "Kattintással, galériából vagy kamerából is hozzáadható. A kiválasztott képméret a helyi előkészítéskor érvényesül." : "Húzza ide a fájlokat, vagy kattintson a fájlválasztó megnyitásához. PDF, Office, CAD/BIM, ZIP és képfájlok is támogatottak."} progress={totalProgress} onFiles={addFiles} onExternalOpen={() => goToStep(1)}/></div>
    ...
    {queue.length ? <div className="mt-5">
      ...
      <div className="mb-4 rounded-2xl border border-cyan-100 bg-cyan-50/60 p-4"><p className="text-xs font-black uppercase tracking-[.12em] text-cyan-800">3. Ellenőrzés</p><h3 className="mt-1 text-lg font-black text-slate-950">{imageOnly ? "Nézze át a képeket" : "Nézze át a fájlokat"}</h3><p className="mt-1 text-sm leading-6 text-slate-600">{imageOnly ? "Itt törölhet, megjegyzést írhat vagy képcsoportot válthat. Semmi nem vész el, ha visszalép." : "Itt ellenőrizheti a fájlneveket, törölhet és megjegyzést írhat. Semmi nem vész el, ha visszalép."}</p></div>
      ...
      <div className="mt-4 rounded-2xl border border-emerald-100 bg-emerald-50/60 p-4"><p className="text-xs font-black uppercase tracking-[.12em] text-emerald-800">6. Lezárás</p><h3 className="mt-1 text-lg font-black text-slate-950">A küldemény készen áll a befejezésre</h3><p className="mt-1 text-sm leading-6 text-slate-600">{queue.length} {imageOnly ? "kép" : "fájl"} van a csomagban. A lezárás csak az alábbi külön megerősítéssel indul el.</p></div>
    ...
    {!delivered && queue.length === 0 && workflowStep >= 2 ? <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4"><strong className="text-sm text-amber-950">Még nincs kiválasztott {imageOnly ? "kép" : "fájl"}.</strong><p className="mt-1 text-xs leading-5 text-amber-900">Lépjen vissza a {imageOnly ? "Képek" : "Fájlok"} pontra, és adjon hozzá legalább egy {imageOnly ? "képet" : "fájlt"}.</p><button type="button" onClick={() => goToStep(1)} className="mt-3 min-h-11 rounded-xl bg-amber-800 px-4 py-2.5 text-xs font-black text-white">Vissza a {imageOnly ? "képekhez" : "fájlokhoz"}</button></div> : null}

    {delivered ? <div className="mt-5 rounded-2xl border border-emerald-300 bg-emerald-50 p-5 text-emerald-950"><CheckCircle2 size={24}/><h3 className="mt-2 text-lg font-black">A küldemény lezárva</h3><p className="mt-1 text-sm leading-6">A fájlcsomag véglegesítése befejeződött. Az e-mail kézbesítés eredménye az állapotüzenetben látható.</p><div className="mt-4 flex flex-wrap gap-3">{onStartNewTransfer ? <button type="button" onClick={onStartNewTransfer} className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-5 py-3 text-sm font-black text-white"><RotateCcw size={16}/> Új küldemény</button> : null}{onClose ? <button type="button" onClick={onClose} className="inline-flex items-center gap-2 rounded-xl border border-emerald-300 bg-white px-5 py-3 text-sm font-black text-emerald-900"><ArrowLeft size={16}/> Bezárás / kezdőlap</button> : null}</div></div> : null}
