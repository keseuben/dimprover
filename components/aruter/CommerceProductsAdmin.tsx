...
export function CommerceProductsAdmin() {
  ...
  async function savePrice(event: FormEvent) {
    ...
    if (!variantId || !/^\d+$/.test(amount)) { setError("A nettó egységár egész Ft érték legyen."); return; }
    setSavingPrice(true); setError(null);
    try {
      const response = await fetch("/api/v1/commerce/prices", {
        method: "POST", headers: { "content-type": "application/json" },
        body: JSON.stringify({ variantId, currency: "HUF", amountMinor: amount, vatRateBasisPoints: 2700 }),
      });
      const result = await response.json() as ApiResult<unknown>;
