...
import { Archive, Check, ClipboardCopy, Download, Eye, FileArchive, FileImage, FileText, Filter, LoaderCircle, Search, UploadCloud, X } from "lucide-react";
...
export default function DevelopmentResourcesDrawer({ open, onClose, resources, health, onReload }: {
  ...
  return <div className={styles.drawerLayer} role="presentation">
    ...
    <aside className={`${styles.drawer} ${styles.drawerExtraWide}`} aria-label="Fejlesztési Tár">
      ...
      <div className={styles.resourceTabs}><button type="button" data-active={tab === "resources"} onClick={() => setTab("resources")}>SEGÉDANYAGOK</button><button type="button" data-active={tab === "handoffs"} onClick={() => setTab("handoffs")}>ÁTADÁSOK <b>{handoffs.length || ""}</b></button></div>
      <div className={styles.drawerBody}>
        {tab === "resources" ? <>
          ...
          <div className={styles.resourceList}>{visible.map((item) => { const inline = ["pdf","png","jpg","jpeg","webp","txt","md","json","csv"].includes(item.extension); return <article key={item.id} className={`${item.requiredBeforeDevelopment ? styles.resourceRequired : ""} ${item.priority === "critical" ? styles.resourceCritical : ""}`}><div className={styles.resourceIcon}>{iconFor(item)}</div><div className={styles.resourceMain}><div><strong>{item.title}</strong><b>{DOC_LABELS[item.documentType]}</b>{item.requiredBeforeDevelopment ? <b>KÖTELEZŐ</b> : null}{item.priority !== "normal" ? <b>{item.priority === "critical" ? "KRITIKUS" : "FONTOS"}</b> : null}</div><span>{item.originalName} · {formatBytes(item.sizeBytes)} · {item.module}{item.version ? ` · ${item.version}` : ""}</span>{item.description ? <p>{item.description}</p> : null}<small>SHA-256 {item.sha256.slice(0,20)}… · {item.tags.join(" · ") || "nincs címke"}</small></div><div className={styles.resourceActions}>{inline ? <button type="button" onClick={() => void openResource(item,true)} title="Előnézet"><Eye size={15}/></button> : null}<button type="button" onClick={() => void openResource(item,false)} title="Letöltés"><Download size={15}/></button><button type="button" onClick={() => void copyHandoff(item)} title="Kontextus másolása">{copied === item.id ? <Check size={15}/> : <ClipboardCopy size={15}/>}</button><button type="button" onClick={() => void patch(item,{ requiredBeforeDevelopment: !item.requiredBeforeDevelopment })} title="Kötelező jelölés"><FileText size={15}/></button><button type="button" onClick={() => { if (window.confirm("Archiválódjon?")) void patch(item,{ archived:true }); }} title="Archiválás"><Archive size={15}/></button></div></article>; })}{!visible.length ? <div className={styles.railEmpty}>Nincs megjeleníthető segédanyag.</div> : null}</div>
        </> : <>
