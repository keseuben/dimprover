...
import { Archive, ArrowRightLeft, Check, ChevronDown, ChevronUp, ClipboardCopy, Download, Eye, FileArchive, FileImage, FileText, Filter, LoaderCircle, Search, UploadCloud, X } from "lucide-react";
import { useEffect, useMemo, useRef, useState } from "react";
import type { DevelopmentResource, ResourceHealth } from "./types";
import styles from "./DeveloperConsole.module.css";

type Handoff = {
  id: string; schemaVersion?: number; fileName?: string; chatSessionId: string; chatTitle: string; workerCode: string; mainProject: string; project: string; module: string; contextModule: string; developmentArea?: string;
  taskId: string; taskTitle: string; liveNextTaskId?: string; liveNextTaskTitle?: string; startedAt: string; finishedAt: string; durationMinutes: number; status: "COMPLETED" | "PARTIAL" | "BLOCKED" | "FAILED";
...
export default function DevelopmentResourcesDrawer({ open, onClose, resources, health, onReload }: {
  ...
  async function downloadStoredHandoff(item: Handoff) {
    setBusy(`handoff:${item.id}`); setMessage("");
    try {
      const response = await fetch(`/api/dev/console/handoffs/${encodeURIComponent(item.id)}`, { headers: adminHeaders(), cache: "no-store" });
      if (!response.ok) { const payload = await response.json().catch(() => null) as { error?: string } | null; throw new Error(payload?.error || "Az átadó nem tölthető le."); }
      const blob = await response.blob(); const objectUrl = URL.createObjectURL(blob); const anchor = document.createElement("a");
      anchor.href = objectUrl; anchor.download = item.fileName || `${item.chatSessionId}_${item.workerCode}_atado.md`; document.body.appendChild(anchor); anchor.click(); anchor.remove();
      window.setTimeout(() => URL.revokeObjectURL(objectUrl), 2_000); setMessage(`${anchor.download} letöltése elindult.`);
    } catch (error) { setMessage(error instanceof Error ? error.message : "Az átadó nem tölthető le."); } finally { setBusy(""); }
  }
    ...
    <aside className={`${styles.drawer} ${styles.drawerExtraWide}`} aria-label="Fejlesztési Tár">
      ...
      <div className={styles.drawerBody}>
        {tab === "resources" ? <>
          ...
          <div className={styles.resourceUploadToggle}>
            <button type="button" aria-expanded={uploadPanelOpen} onClick={() => setUploadPanelOpen((value) => !value)}>
              <UploadCloud size={16} /><span>{uploadPanelOpen ? "FELTÖLTÉS / METAADATOK ELREJTÉSE" : "ÚJ SEGÉDANYAG FELTÖLTÉSE"}</span>{uploadPanelOpen ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
            </button>
            <small>{uploadPanelOpen ? "A lista a panel alatt külön görgethető." : "A metaadatmezők csak feltöltéskor nyílnak le; a segédanyaglista szabadon látható marad."}</small>
          </div>
          {uploadPanelOpen ? <section className={styles.resourceUploadPanel}>
          ...
          {message ? <div className={styles.drawerNotice}>{message}</div> : null}
          {handoffLoading ? <div className={styles.railEmpty}><LoaderCircle size={20} className={styles.spin}/> Átadások betöltése…</div> : <div className={styles.handoffGroups}>{groupedHandoffs.map(([group,items]) => <section key={group}><header><strong>{group}</strong><span>{items.length} átadó</span></header>{items.map((item) => <article key={item.id} className={styles.handoffCard} draggable data-handoff-id={item.id}><div className={styles.handoffCardTop}><div><strong>{item.chatSessionId} · {item.chatTitle}</strong><span>{item.workerCode} · {item.mainProject} / {item.project} / {item.module}{item.contextModule ? ` / ${item.contextModule}` : ""}{item.developmentArea ? ` · ${item.developmentArea}` : ""}</span></div><b data-status={item.status}>{item.status}</b></div><div className={styles.handoffFacts}><span>{formatDate(item.startedAt)} → {formatDate(item.finishedAt)}</span><strong>{duration(item.durationMinutes)}</strong><span>Task: {item.taskId}</span><span>{item.taskTitle}</span></div><p>{item.summary}</p><strong className={styles.handoffFileName}>{item.fileName || `${item.chatSessionId}_${item.workerCode}_atado.md`}</strong><div className={styles.handoffCardActions}><button type="button" disabled={busy === `handoff:${item.id}`} onClick={() => void downloadStoredHandoff(item)}><Download size={15}/><span>{busy === `handoff:${item.id}` ? "Letöltés…" : "Letöltés .MD"}</span></button></div><small>{item.testsSummary || "tesztadat nincs"} · PROD {item.productionAccess} · SHA {item.sha256.slice(0,16)}…</small></article>)}</section>)}</div>}
