      ...
      <div className={styles.browser}>
        ...
        <div className={styles.documentPanel}>
          ...
          {browserViewMode === "review" && <section className={styles.reviewHost}>
            <header className={styles.reviewHeader}><div><span>Tervellenőrzés</span><strong>{reviewRows.length} terv</strong></div><div className={styles.reviewLegend}>✓ megfelelő · ⚠ javítandó · ↩ visszaadva · ◷ folyamatban · + új · ● módosult · ↪ áthelyezve · ✕ nem található · — nincs adat</div></header>
            ...
            <div className={styles.reviewTableWrap}><table className={styles.reviewTable}><thead><tr><th>Terv</th><th>Szakág</th><th>Témakör</th><th>Ellenőrzés</th><th>Eredmény</th><th>Észrevételek</th><th>Workflow állapot</th><th>Belső megjegyzés</th><th>Megrendelő</th><th>Megrendelői megjegyzés</th><th>Revízióváltozás</th></tr></thead><tbody>{reviewRows.map((row) => <tr key={row.document.id}><td><button type="button" className={styles.reviewName} onClick={() => setSelectedDocumentId(row.document.id)}>{row.document.name}</button></td><td>{row.effectiveDiscipline || "—"}</td><td>{row.effectiveTopic || "—"}</td><td><button type="button" className={styles.reviewSymbol} title={row.checked || "Nincs ellenőrzési adat"} onClick={() => setSelectedDocumentId(row.document.id)}>{reviewMark(row.checked)}</button></td><td><button type="button" className={styles.reviewSymbol} title={row.result || "Nincs eredmény"} onClick={() => setSelectedDocumentId(row.document.id)}>{reviewMark(row.result)}</button></td><td><button type="button" className={styles.reviewSymbol} title={row.observations || "Nincs észrevétel"} onClick={() => setSelectedDocumentId(row.document.id)}>{row.observationCount || "—"}</button></td><td><button type="button" className={styles.reviewSymbol} title={row.workflow || "Nincs workflow állapot"} onClick={() => setSelectedDocumentId(row.document.id)}>{reviewMark(row.workflow)}</button></td><td><button type="button" className={styles.reviewSymbol} title={row.internalNote || "Nincs belső megjegyzés"} onClick={() => setSelectedDocumentId(row.document.id)}>{row.internalNote ? "●" : "—"}</button></td><td><button type="button" className={styles.reviewSymbol} title={row.customer || "Nincs megrendelői jóváhagyás"} onClick={() => setSelectedDocumentId(row.document.id)}>{reviewMark(row.customer)}</button></td><td><button type="button" className={styles.reviewSymbol} title={row.customerNote || "Nincs megrendelői megjegyzés"} onClick={() => setSelectedDocumentId(row.document.id)}>{row.customerNote ? "●" : "—"}</button></td><td><button type="button" className={styles.reviewSymbol} title={row.revisionChange || "Nincs revízióváltozás"} onClick={() => setSelectedDocumentId(row.document.id)}>{revisionMark(row.revisionChange)}</button></td></tr>)}</tbody></table></div>
            <DetailsPanel projectId={projectId} document={selectedDocument} details={details} loading={detailsLoading} busy={busy} canWrite={canWrite} canComment={canComment} canApprove={canApprove} securityReady={securityScannerReady} securityLabel={securityScannerReady ? `${health?.security?.engine || "ClamAV"}` : health?.security?.errorCode || "Scanner nem elérhető"} onScan={async () => { if (selectedDocument) await scanDocumentVersion(selectedDocument); }} onReview={async (action) => { if (selectedDocument) await reviewDocumentVersion(selectedDocument, action); }} onSaveMetadata={saveSelectedMetadata} onSaveNote={saveSelectedNote} onEnsureQr={ensureSelectedQr} onDownload={async () => { if (selectedDocument) await downloadDocument(selectedDocument); }} focusTab="review" inheritedDiscipline={selectedDocument ? effectiveFolderClassification.get(selectedDocument.folderId)?.discipline || "" : ""} inheritedTopic={selectedDocument ? effectiveFolderClassification.get(selectedDocument.folderId)?.topic || "" : ""} />
            </div>
          </section>}
