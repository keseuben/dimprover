...
export default function DriveWorkspace({ projectId, permissions = [] }: Props) {
  ...
  const reviewRows = useMemo(() => { const query = reviewSearch.trim().toLocaleLowerCase("hu-HU"); return allReviewRows.filter((row) => (!query || [row.planTitle, row.document.name, row.effectiveDiscipline, row.effectiveTopic, row.observations, row.internalNote, row.customerNote].some((value) => String(value || "").toLocaleLowerCase("hu-HU").includes(query))) && (reviewDiscipline === "all" || row.effectiveDiscipline === reviewDiscipline) && (reviewTopic === "all" || row.effectiveTopic === reviewTopic) && (reviewStatus === "all" || (reviewStatus === "not-approved" ? !isApprovedReviewValue(row.customer) : row.workflow === reviewStatus))); }, [allReviewRows, reviewDiscipline, reviewTopic, reviewStatus, reviewSearch]);
      ...
      <div className={styles.browser}>
        ...
        <div className={styles.documentPanel}>
          ...
          {browserViewMode === "review" && <section className={styles.reviewHost}>
            <header className={styles.reviewHeader}><div><span>Tervellenőrzés</span><strong>{reviewRows.length} / {allReviewRows.length} terv</strong></div><div className={styles.reviewLegend}>✓ megfelelő · ⚠ javítandó · ↩ visszaadva · ◷ folyamatban · + új · ● módosult · ↪ áthelyezve · ✕ nem található · — nincs adat</div></header>
