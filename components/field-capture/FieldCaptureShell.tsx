...
export default function FieldCaptureShell({ identity }: { identity?: TerepIdentityContext }) {
  ...
  const remeasureOrientation = async (item: FieldCaptureItem) => {
    const requesting = { ...item.orientation, enabled: true, status: "REQUESTING" as const, detail: "Tájolás újramérés folyamatban…" };
      ...
      <div className="mx-auto w-full max-w-2xl px-3 py-4 sm:px-5 sm:py-7">
        <header className="overflow-hidden rounded-[1.8rem] border border-cyan-200 bg-white shadow-sm">
          <div className="bg-gradient-to-r from-slate-950 via-cyan-950 to-teal-900 p-4 text-white sm:p-5">
            <div className="flex items-center gap-3"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-white/10 text-cyan-200"><Camera size={24} /></span><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[.17em] text-cyan-200">DIMPRO Drop · Terep</p><h1 className="mt-1 text-xl font-black sm:text-2xl">Terep <span className="text-cyan-300">DEV</span></h1><p className="mt-1 text-[11px] text-cyan-100">V{FIELD_CAPTURE_VERSION} · P0–P6 · offline capture + GPS/tájolás</p></div></div>
          </div>
        ...
        <section className="mt-4">
          ...
          {items.length ? <div className="space-y-3">{items.map((item) => <CapturePreviewCard key={item.id} item={item} onNoteChange={(note) => void updateNote(item.id, note)} onVoiceCommit={(note) => void updateNote(item.id, note)} onRemeasureLocation={() => void remeasureLocation(item)} onRemeasureOrientation={() => void remeasureOrientation(item)} onDelete={() => void deleteItem(item)} />)}</div> : <div className="rounded-[1.8rem] border border-dashed border-slate-300 bg-white p-8 text-center"><Camera size={34} className="mx-auto text-slate-300" /><strong className="mt-3 block text-sm text-slate-700">Még nincs terepi kép ebben a munkamenetben</strong><p className="mt-1 text-xs leading-5 text-slate-500">Az első kép után itt jelenik meg a fotó, a megjegyzés, a helyi mentési állapot és a képenkénti metaadatkérés.</p></div>}
        </section>
        <section className="mt-4 rounded-[1.5rem] border border-slate-200 bg-white p-3"><div className="flex items-start gap-2"><ShieldCheck size={17} className="mt-0.5 shrink-0 text-emerald-700" /><div><strong className="text-xs text-slate-800">Terep biztonsági alapelv</strong><p className="mt-1 text-[11px] leading-5 text-slate-500">A helyi queue nem tárol Drop Send-kódot, PIN-t vagy nyers upload capability tokent. A GPS és tájolás külön strukturált capture rekord, nem EXIF.</p></div></div></section>
      </div>
