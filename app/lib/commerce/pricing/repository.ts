...
function parseAmountMinor(value:unknown){const raw=text(value);if(!/^\d+$/.test(raw))throw new CommercePricingError("Az ár csak nemnegatív egész minor-unit érték lehet.","COMMERCE_PRICE_AMOUNT_INVALID",400);const amount=BigInt(raw);if(amount>BigInt(Number.MAX_SAFE_INTEGER))throw new CommercePricingError("Az ár túl nagy.","COMMERCE_PRICE_AMOUNT_TOO_LARGE",400);return raw;}
...
export async function listCommercePrices(context:CommerceContext,input:{variantId?:unknown;currency?:unknown;limit?:number}={}){
  ...
  let query=client.from("commerce_prices").select("id,organization_id,variant_id,currency,amount_minor,vat_rate_basis_points,valid_from,valid_until,status,created_at,updated_at")
