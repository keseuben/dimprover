...
async function verifySourceWarehouse(context:CommerceContext,sourceId:string,warehouseId:string){
  const client=createCommerceAdminClient();
  const source=await client.from("commerce_inventory_sources").select("id,warehouse_id,source_type,active").eq("organization_id",context.organizationId).eq("id",sourceId).is("deleted_at",null).maybeSingle();
  ...
  const warehouse=await client.from("commerce_warehouses").select("id,active").eq("organization_id",context.organizationId).eq("id",warehouseId).is("deleted_at",null).maybeSingle();
...
export async function listCommerceGoodsReceipts(context:CommerceContext,input:{status?:unknown;limit?:number}={}){
  ...
  let query=client.from("commerce_goods_receipts").select("*").eq("organization_id",context.organizationId).is("deleted_at",null).order("received_at",{ascending:false}).order("created_at",{ascending:false}).limit(Math.max(1,Math.min(200,Math.floor(input.limit||50))));
...
export async function getCommerceGoodsReceipt(context:CommerceContext,receiptIdInput:unknown){
  ...
  const [receipt,items]=await Promise.all([
    client.from("commerce_goods_receipts").select("*").eq("organization_id",context.organizationId).eq("id",receiptId).is("deleted_at",null).maybeSingle(),
    client.from("commerce_goods_receipt_items").select("*").eq("organization_id",context.organizationId).eq("receipt_id",receiptId).is("deleted_at",null).order("created_at",{ascending:true}).order("id",{ascending:true}),
  ]);
