...
function quantity(value:unknown){let result:string;try{result=normalizeDecimal(text(value));}catch{throw new CommerceReceivingError("A mennyiség legfeljebb 6 tizedesjegyű szám lehet.","COMMERCE_RECEIPT_QUANTITY_INVALID",400);}if(compareDecimal(result,"0")<=0)throw new CommerceReceivingError("A mennyiségnek pozitívnak kell lennie.","COMMERCE_RECEIPT_QUANTITY_INVALID",400);return result;}
function costMinor(value:unknown){if(value===undefined||value===null||text(value)==="")return null;const raw=text(value);if(!/^\d+$/.test(raw))throw new CommerceReceivingError("Az egységköltség nemnegatív egész minor-unit érték legyen.","COMMERCE_RECEIPT_COST_INVALID",400);return raw;}
...
async function itemPayload(context:CommerceContext,receiptId:string,input:Record<string,unknown>){
  ...
  return{organization_id:context.organizationId,receipt_id:receiptId,variant_id:variantId,stock_status:stockStatus,quantity:quantity(input.quantity),unit,unit_cost_minor:costMinor(input.unitCostMinor),currency,lot_code:nullableText(input.lotCode),expiry_date:dateOnly(input.expiryDate),notes:nullableText(input.notes)};
}
