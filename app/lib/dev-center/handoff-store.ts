...
export async function listDevelopmentHandoffs(filters: Record<string, string> = {}) {
  ...
  if (query) items = items.filter((item) => `${item.chatSessionId} ${item.chatTitle} ${item.workerCode} ${item.mainProject} ${item.project} ${item.module} ${item.contextModule} ${item.taskId} ${item.taskTitle} ${item.summary} ${item.tags.join(" ")}`.toLocaleLowerCase("hu-HU").includes(query));
...
export async function saveDevelopmentHandoff(input: Record<string, unknown>) {
  ...
  const moduleKey = slug(input.contextModule || input.module);
  const date = new Date(finishedAt);
  const yyyy = String(date.getUTCFullYear());
  const mm = String(date.getUTCMonth() + 1).padStart(2, "0");
  const dd = String(date.getUTCDate()).padStart(2, "0");
  const dir = path.join(root, "history", yyyy, mm, dd, workerCode);
  const fileName = `${String(input.chatSessionId).replace(/[^0-9A-Za-z_-]/g, "-")}_${moduleKey}_${id.slice(-8)}.md`;
  const filePath = path.join(dir, fileName);
  const body = text(input.body, 200_000);
  const front = [
    "---", "schemaVersion: 1", `handoffId: ${q(id)}`, `chatSessionId: ${q(input.chatSessionId)}`, `chatTitle: ${q(input.chatTitle)}`,
    ...
    `taskId: ${q(input.taskId)}`, `taskTitle: ${q(input.taskTitle)}`, `startedAt: ${q(startedAt)}`, `finishedAt: ${q(finishedAt)}`, `durationMinutes: ${durationMinutes}`,
