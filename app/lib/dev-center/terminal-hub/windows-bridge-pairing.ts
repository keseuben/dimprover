...
function pairingHash(pairingId: string, code: string) { return createHmac("sha256", pairingSecret()).update(`${pairingId}:${normalizeCode(code)}`, "utf8").digest("hex"); }
function randomToken() { return randomBytes(TOKEN_BYTES).toString("base64url"); }
function safeHashEqual(left: string, right: string) {
  const a = Buffer.from(left, "hex"); const b = Buffer.from(right, "hex");
  return a.length === b.length && a.length > 0 && timingSafeEqual(a, b);
}
function normalizeCode(value: string) { return value.toUpperCase().replace(/[^0-9A-Z]/g, "").slice(0, 16); }
function randomPairingCode() {
  const bytes = randomBytes(10); let code = "";
...
export async function createWindowsBridgePairing(actor = "BENJADMIN") {
  ...
  const client = db(); const pairingId = randomUUID(); const code = randomPairingCode();
  const expiresAt = new Date(Date.now() + WINDOWS_BRIDGE_PAIRING_MAX_AGE_SECONDS * 1000).toISOString();
  const result = await client.from("dev_center_windows_bridge_pairings").insert({ id: pairingId, code_hash: pairingHash(pairingId, code), status: "pending", expires_at: expiresAt, attempt_count: 0, max_attempts: PAIRING_ATTEMPT_LIMIT, created_by: actor, metadata: { protocolVersion: WINDOWS_BRIDGE_PROTOCOL_VERSION } }).select("id,expires_at,max_attempts").single();
  if (result.error) throw new WindowsBridgePairingError("PAIRING_CREATE_FAILED", result.error.message, 500);
  await audit(client, { action: "WINDOWS_BRIDGE_PAIRING_CREATED", entityType: "windows_bridge_pairing", entityId: pairingId, actor, summary: "Windows Bridge egyszer használatos pairing létrehozva.", metadata: { expiresAt, maxAttempts: PAIRING_ATTEMPT_LIMIT } });
  return { pairingId, code, expiresAt, maxAttempts: PAIRING_ATTEMPT_LIMIT, protocolVersion: WINDOWS_BRIDGE_PROTOCOL_VERSION };
}
...
export async function claimWindowsBridgePairing(input: { pairingId: string; code: string; hello: WindowsBridgeAgentHello }) {
  ...
  const suppliedHash = pairingHash(pairingId, input.code || "");
  if (!safeHashEqual(String(pairing.code_hash), suppliedHash)) {
    const attempts = Number(pairing.attempt_count || 0) + 1; const max = Number(pairing.max_attempts || PAIRING_ATTEMPT_LIMIT);
  ...
  const deviceId = String(deviceResult.data.id); const claimToken = randomToken(); const claimHash = sha256(claimToken);
...
export async function pollWindowsBridgeClaim(pairingId: string, claimToken: string) {
  assertPairingEnabled(); const client = db(); const claimHash = sha256(claimToken || "");
  ...
  if (!pairing || !pairing.claim_token_hash || !safeHashEqual(String(pairing.claim_token_hash), claimHash)) throw new WindowsBridgePairingError("CLAIM_TOKEN_INVALID", "Érvénytelen pairing claim token.", 401);
  ...
  const deviceToken = randomToken(); const tokenHash = sha256(deviceToken); const sessionId = randomUUID();
...
export async function authenticateWindowsBridgeDevice(token: string, agentId?: string) {
  assertBridgeEnabled(); const tokenHash = sha256(token || ""); const client = db();
