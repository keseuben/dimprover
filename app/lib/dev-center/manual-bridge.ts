...
export function buildManualBridgeHandoff(input: {
  ...
  const prompt = [
    ...
    "BenjAdmin a végső emberi döntéshozó, fejlesztési vezető, rendszergazda és rendszertulajdonos. Az AI worker a kijelölt scope-on belül hajtja végre BenjAdmin utasítását.",
    "MUNKAFELVÉTEL KÖTELEZŐ: a task tényleges felvételekor írd ki a chatben: MUNKAFELVÉTEL: YYYY.MM.DD. HH:MM (Europe/Budapest). A BENJADMIN Plus pull/audit adata legyen a szerveroldali mérvadó időbélyeg.",
    "MUNKA VISSZAADÁSA KÖTELEZŐ: befejezéskor, blokkoláskor vagy átadáskor írd ki: MUNKA VISSZAADVA: YYYY.MM.DD. HH:MM (Europe/Budapest), valamint az eltelt munkaidőt, task állapotot, fő eredményt és következő lépést. Ne maradjon csendben befejezett munkamenet.",
