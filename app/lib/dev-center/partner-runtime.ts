...
export async function getPartnerRuntimeIsolationStatus() {
  ...
  const hashReady = await tokenHashReady();
  ...
  const ready = rootReady && directoriesReady && hashReady && markerReady;
  ...
  return {
    ...
    blockers: [
      ...
      ...(marker?.internalRootProtected !== true ? ["OUTMIN_INTERNAL_ROOT_NOT_PROTECTED"] : []),
