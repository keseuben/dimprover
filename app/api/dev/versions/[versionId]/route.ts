...
export async function PATCH(request: NextRequest, context: { params: Promise<{ versionId: string }> }) {
  if (!(await isDevCenterAuthorized(request.headers, true))) {
    return NextResponse.json({ ok: false, error: "Nincs jogosultság a fejlesztési verzió módosításához." }, { status: 401 });
  }
  const { versionId } = await context.params;
  const result = await updateDevVersion(versionId, await request.json().catch(() => ({})));
