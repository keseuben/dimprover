...
import { analyzeExternalAiWorkerTask } from "@/app/lib/dev-center/ai-worker/v1";
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await analyzeExternalAiWorkerTask(id);
    return NextResponse.json(result, { status: result.ok ? 200 : 400, headers: { "cache-control": "no-store" } });
  } catch (error) {
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "A technikai scope nem elemezhető." }, { status: 500 });
  }
}
