...
export async function GET(request: NextRequest, context: { params: Promise<{ sessionId: string }> }) {
  ...
  try { const {sessionId}=await context.params; const after=Number(new URL(request.url).searchParams.get("after")||0); const result=readTerminalOutput(OWNER,sessionId,after); return NextResponse.json({ok:true,session:result.session,chunks:toSanitizedTerminalChunks(result.chunks)},{headers:{"cache-control":"no-store"}}); }
