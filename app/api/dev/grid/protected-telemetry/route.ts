...
export async function POST(request: NextRequest) {
  ...
  try {
    const sample = sanitizeProtectedTelemetryPayload(payload);
    if (!(await isProtectedTelemetryAuthorized(request.headers, sample.nodeId))) return json({ ok: false, code: "PROTECTED_TELEMETRY_UNAUTHORIZED", error: "Protected telemetry hitelesítés sikertelen." }, 401);
    const stored = await storeProtectedTelemetry(sample);
