...
import { updateDevEngineTaskDevelopmentMap } from "@/app/lib/dev-center/engine-repository";
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ taskId: string }> }) {
  ...
  try {
    ...
    const body = await request.json().catch(() => ({})) as { nodeId?: string; workItem?: string };
    const result = await updateDevEngineTaskDevelopmentMap({ taskId, nodeId: String(body.nodeId || ""), workItem: body.workItem, updatedBy: "BenjAdmin" });
