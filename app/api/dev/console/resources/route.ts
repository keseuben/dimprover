...
import { developmentResourceHealth, listDevelopmentResources, saveDevelopmentResource, updateDevelopmentResource } from "@/app/lib/dev-center/development-resources";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const saved = [];
    for (const file of files) saved.push(await saveDevelopmentResource({ module: moduleCode, title: files.length === 1 ? title : "", description, tags, priority, source, version, requiredBeforeDevelopment, file }));
...
export async function PATCH(request: NextRequest) {
  if (!(await isDevCenterAuthorized(request.headers))) return unauthorized();
  try {
    const body = await request.json() as { id?: string; title?: string; description?: string; tags?: string[]; priority?: "normal" | "important" | "critical"; source?: string; version?: string; requiredBeforeDevelopment?: boolean; archived?: boolean };
