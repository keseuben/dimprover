...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  if (!(await isDevCenterAuthorized(request.headers, true))) return Response.json({ ok: false, error: "Nincs jogosultság a segédanyaghoz." }, { status: 401 });
  try {
    const { id } = await context.params;
    const { resource, bytes } = await getDevelopmentResourceContent(id);
