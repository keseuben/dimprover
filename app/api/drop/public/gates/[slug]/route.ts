...
export async function GET(_request: NextRequest, context: Context) {
  try { assertDropFeatureEnabled("submissionGateEnabled"); const { slug } = await context.params; return NextResponse.json({ ok: true, version: "DROP 1.2.13", gate: safeGate(await getDropSubmissionGateBySlug(slug)) }, { headers: dropNoStoreHeaders() }); }
...
export async function POST(request: NextRequest, context: Context) {
  try {
    ...
    const response = NextResponse.json({ ok: true, version: "DROP 1.2.13", gate: safeGate(gate), session: { expiresAt: session.record.expiresAt } }, { status: 201, headers: dropNoStoreHeaders() });
