...
export async function GET(request: NextRequest) {
  ...
  try { return NextResponse.json({ ok: true, version: "DROP 1.2.11", gates: await listDropSubmissionGates() }, { headers: dropNoStoreHeaders() }); }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, version: "DROP 1.2.11", gate, publicUrl: `${base}/bekuldes/${encodeURIComponent(gate.slug)}` }, { status: 201, headers: dropNoStoreHeaders() });
  } catch (error) { return dropErrorResponse(error); }
}
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, version: "DROP 1.2.11", gate: await setDropSubmissionGateStatus(id, status) }, { headers: dropNoStoreHeaders() });
  } catch (error) { return dropErrorResponse(error); }
}
