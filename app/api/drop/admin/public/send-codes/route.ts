...
export async function GET(request: NextRequest) {
  ...
  try { return NextResponse.json({ ok: true, version: "DROP 1.2.13", sendCodes: await listDropSendCodes() }, { headers: dropNoStoreHeaders() }); }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, version: "DROP 1.2.13", created, warning: "A nyers küldési kód csak ebben a válaszban jelenik meg. Mentse el biztonságosan." }, { status: 201, headers: dropNoStoreHeaders() });
  } catch (error) { return dropErrorResponse(error); }
}
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, version: "DROP 1.2.13", sendCode: await setDropSendCodeStatus(id, status) }, { headers: dropNoStoreHeaders() });
  } catch (error) { return dropErrorResponse(error); }
}
