...
import { initializeFieldCaptureDropUpload } from "@/app/lib/field-capture/dropUploadAdapter";
...
export async function POST(
  ...
  try {
    ...
    if (!UUID_RE.test(packageId)) {
      throw new DimproIdentityError(
        "Érvényes Drop csomagazonosító szükséges.",
        "FIELD_CAPTURE_DROP_PACKAGE_ID_INVALID",
        400,
      );
    }
    const result = await initializeFieldCaptureDropUpload({
      session,
      itemId,
      entitlementId: authorized.context.entitlement.id,
      packageId,
      actorName: authorized.context.user.fullName,
      actorEmail: authorized.context.user.email,
      rulesAccepted: body.rulesAccepted === true,
      rulesVersion: typeof body.rulesVersion === "string" ? body.rulesVersion : "",
      rulesAcceptedAt: typeof body.rulesAcceptedAt === "string" ? body.rulesAcceptedAt : "",
    });
