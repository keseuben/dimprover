...
import { addDriveDocumentVersion } from "@/app/lib/drive-core/store";
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    const result = await addDriveDocumentVersion(projectId, documentId, input, access.actor.userId);
    return NextResponse.json(result, { status: result.ok ? 201 : result.conflict ? 409 : 400, headers: { "cache-control": "no-store" } });
