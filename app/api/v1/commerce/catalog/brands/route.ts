...
export async function GET(request:NextRequest){
  try{
    ...
    const data=await listCommerceCatalog(context,"brands",{query:request.nextUrl.searchParams.get("q")||undefined,active:activeParam===null?undefined:activeParam==="true"});
...
export async function POST(request:NextRequest){
  try{
    ...
    const data=await createCommerceCatalogItem(context,"brands",body);
