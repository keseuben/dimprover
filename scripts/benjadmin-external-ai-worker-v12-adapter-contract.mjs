...
try{
 ...
 Object.assign(process.env,{OPENAI_API_KEY:"test-openai-secret",ANTHROPIC_API_KEY:"test-anthropic-secret",DIMPRO_EXTERNAL_AI_OPENAI_MODEL:"openai-test-model",DIMPRO_EXTERNAL_AI_CLAUDE_MODEL:"claude-test-model",DIMPRO_EXTERNAL_AI_PROVIDER_EXECUTION_ENABLED:"true",DIMPRO_EXTERNAL_AI_OPENAI_INPUT_HUF_PER_MTOKEN:"1000",DIMPRO_EXTERNAL_AI_OPENAI_OUTPUT_HUF_PER_MTOKEN:"2000",DIMPRO_EXTERNAL_AI_CLAUDE_INPUT_HUF_PER_MTOKEN:"1200",DIMPRO_EXTERNAL_AI_CLAUDE_OUTPUT_HUF_PER_MTOKEN:"2400"});
 const configuredProbes=await adapters.probeWorkerModelAdapters();
 check("OpenAI transport + secret + model + pricing + gate READY",()=>{const p=configuredProbes.find(x=>x.provider==="openai");assert.equal(p?.executionImplemented,true);assert.equal(p?.pricingConfigured,true);assert.equal(p?.ready,true)});
 check("Claude transport + secret + model + pricing + gate READY",()=>{const p=configuredProbes.find(x=>x.provider==="anthropic");assert.equal(p?.executionImplemented,true);assert.equal(p?.pricingConfigured,true);assert.equal(p?.ready,true)});
 await checkAsync("AUTO valódi READY providert választ",async()=>assert.notEqual(await adapters.resolveWorkerModelAdapter("AUTO","MFORGE"),null));
