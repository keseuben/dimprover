...
check("Planning scheduler presence maps to stage 1",()=>assert.ok(bridge.includes('["analysis", "planning", "prepare", "preparation", "discovery"].includes(phase)')&&bridge.includes("return 1")));
check("Explicit scheduler stage overrides phase fallback",()=>assert.ok(bridge.includes("workStageIndex: item.workStageIndex || stageForPhase(item.phase)")&&!bridge.includes("workItem: item.workItem, workStageIndex: stageForPhase(item.phase)")));
