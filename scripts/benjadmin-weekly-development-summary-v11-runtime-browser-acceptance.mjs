...
try {
  ...
  const project = await db.from("dev_center_projects").insert({ id: projectId, name: `${marker} projekt`, slug: projectSlug, category: "Acceptance", description: "Weekly Summary V1.1 acceptance", status: "active", accent: "cyan", metadata: { marker, productionAccess: "DENY" } });
  ...
  const previousWeekKey = summary?.period?.previousWeekKey;
  result = await api(`/api/dev/console/weekly-summary?projectId=${encodeURIComponent(projectId)}&week=${encodeURIComponent(previousWeekKey)}`);
  const previousSummary = result.payload?.summary;
  check("V1.1 API loads previous Budapest week", result.response.status === 200 && previousSummary?.period?.weekKey === previousWeekKey && previousSummary?.period?.isCurrentWeek === false, JSON.stringify(previousSummary?.period || {}));
  check("V1.1 previous week points forward to current week", previousSummary?.period?.nextWeekKey === summary?.period?.currentWeekKey, JSON.stringify(previousSummary?.period || {}));
  check("V1.1 isolated current-week activities do not leak into previous week", previousSummary?.stats?.activities === 0, JSON.stringify(previousSummary?.stats || {}));
  ...
  check("V1.1 weekly toolbar is visible", Boolean(await page.$('[data-testid="benjadmin-weekly-summary-toolbar"]')));

  await page.click('[data-worker-code="ARMINAI"]');
  await page.waitForFunction(() => document.querySelector('[data-testid="benjadmin-weekly-development-summary"]')?.getAttribute("data-worker-filter") === "ARMINAI", { timeout: 10000 });
  let filterState = await page.evaluate(() => ({
    worker: document.querySelector('[data-testid="benjadmin-weekly-development-summary"]')?.getAttribute("data-worker-filter"),
    count: document.querySelector('[data-testid="benjadmin-weekly-summary-contexts"]')?.getAttribute("data-filtered-count"),
    text: document.querySelector('[data-testid="benjadmin-weekly-summary-contexts"]')?.textContent || "",
    search: window.location.search,
  }));
  check("V1.1 worker filter keeps only ARMINAI context and URL state", filterState.worker === "ARMINAI" && filterState.count === "1" && filterState.text.includes("Kontextus A") && !filterState.text.includes("Kontextus B") && filterState.search.includes("weeklyWorker=ARMINAI"), JSON.stringify(filterState));

  await page.click('[data-stage="3"]');
  await page.waitForFunction(() => document.querySelector('[data-testid="benjadmin-weekly-development-summary"]')?.getAttribute("data-stage-filter") === "3", { timeout: 10000 });
  filterState = await page.evaluate(() => ({
    stage: document.querySelector('[data-testid="benjadmin-weekly-development-summary"]')?.getAttribute("data-stage-filter"),
    count: document.querySelector('[data-testid="benjadmin-weekly-summary-contexts"]')?.getAttribute("data-filtered-count"),
    search: window.location.search,
  }));
  check("V1.1 6/3 filter combines with worker filter and persists", filterState.stage === "3" && filterState.count === "1" && filterState.search.includes("weeklyStage=3"), JSON.stringify(filterState));
  await page.click('[data-stage="3"]');
  await page.click('[data-worker-code="ARMINAI"]');
  ...
  }, { timeout: 30000 }, previousWeekKey);
  const previousUi = await page.evaluate(() => ({
    week: document.querySelector('[data-testid="benjadmin-weekly-development-summary"]')?.getAttribute("data-week-key"),
    text: document.querySelector('[data-testid="benjadmin-weekly-development-summary"]')?.textContent || "",
    search: window.location.search,
  }));
  ...
  await page.waitForSelector('[data-context-key]', { timeout: 30000 });
  const cards = await page.$$('[data-context-key]');
  let contextACard = null;
  for (const card of cards) {
    const text = await card.evaluate((node) => node.textContent || "");
    if (text.includes("Kontextus A")) { contextACard = card; break; }
  }
  if (!contextACard) throw new Error("Kontextus A deep-link card missing");
  await contextACard.click();
  await page.waitForSelector('[data-testid="benjadmin-worker-activity-context-filter"]', { timeout: 10000 });
  const deepLink = await page.evaluate(() => ({
    filter: Boolean(document.querySelector('[data-testid="benjadmin-worker-activity-context-filter"]')),
    search: window.location.search,
  }));
  check("V1.1 context card opens worker drawer with persistent context deep-link", deepLink.filter && deepLink.search.includes("weeklyContext=") && deepLink.search.includes("weeklyDrawerWorker=ARMINAI"), JSON.stringify(deepLink));
  await page.evaluate(() => {
    const section = document.querySelector('[data-testid="benjadmin-worker-activity-context-filter"]');
    const button = section?.querySelector("button");
    if (button instanceof HTMLElement) button.click();
  });
  await page.waitForFunction(() => !document.querySelector('[data-testid="benjadmin-worker-activity-context-filter"]'), { timeout: 10000 });
  check("V1.1 worker drawer context filter can be cleared", true);
