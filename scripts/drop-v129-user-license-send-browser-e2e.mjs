...
const PORT=Number(process.env.DROP_V129_BROWSER_PORT||3120);
...
const fullName=`DROP 1.2.9 Browser User ${stamp}`;
const email=`drop-v129-browser-${stamp}@example.invalid`;
const phone='+36 30 987 6543';
const organizationName=`DIMPRO V129 Browser Org ${stamp}`;
...
try{
 ...
 const r=await page.goto(`${LICENSE}/drive/drop/public-workflows?v129=1`,{waitUntil:'networkidle2',timeout:120000});
 pass('admin-http',r?.status()===200,String(r?.status()));
 await page.waitForFunction(()=>(document.body.textContent||'').includes('DROP 1.2.9')&&(document.body.textContent||'').includes('Új Send entitlement'),{timeout:30000});
 pass('v129-admin-visible',true);
 ...
 await sendPage.evaluateOnNewDocument(()=>{
   ...
   class FakeSpeechRecognition {
     ...
     start(){
       const emit=(text,isFinal,delay)=>setTimeout(()=>{if(this._stopped)return;const result={0:{transcript:text},isFinal};const results={0:result,length:1};this.onresult?.({resultIndex:0,results});},delay);
       emit('teszt',false,35); emit('teszt hangos',false,70); emit('teszt hangos levélüzenet',true,110);
     }
 ...
 const sr=await sendPage.goto(`${DROP}/send?v129-login=1`,{waitUntil:'networkidle2',timeout:120000});
 ...
 const voiceMessageValue=await sendPage.evaluate(()=>[...document.querySelectorAll('textarea')].find(el=>el.placeholder?.includes('mai helyszíni'))?.value||'');
 pass('message-voice-transcript-committed',voiceMessageValue.includes('teszt hangos levélüzenet'),voiceMessageValue);
 pass('message-voice-evolving-result-not-duplicated',!voiceMessageValue.includes('teszt teszt')&&!voiceMessageValue.includes('teszt hangos teszt'),voiceMessageValue);
 ...
 await sendPage.type('input[placeholder="Név"]','Csató Ferenc V129',{delay:15});
 await sendPage.type('input[placeholder="E-mail-cím"]','csato.ferenc.v129.browser@example.invalid',{delay:15});
 ...
 await sendPage.waitForFunction(()=>{const t=document.body.textContent||'';return t.includes('csato.ferenc.v129.browser@example.invalid')&&t.includes('A címjegyzék mentve.');},{timeout:20000});
 const contactRow=await client.from('dimpro_send_recipients').select('id,recipient_name,recipient_email,organization_name,active').eq('entitlement_id',fixture.entitlementId).eq('recipient_email','csato.ferenc.v129.browser@example.invalid').single(); if(contactRow.error)throw contactRow.error;
 ...
 await sendPage.waitForFunction(()=> (document.body.textContent||'').includes('Képek küldése')&&(document.body.textContent||'').includes('DROP 1.2.9'),{timeout:30000});
 ...
 try { await sendPage.waitForFunction(()=> (document.body.textContent||'').includes('Megjegyzés ehhez a képhez'),{timeout:30000}); }
 catch(error){ const debug=await sendPage.evaluate(()=>({body:(document.body.textContent||'').slice(-5000),inputs:[...document.querySelectorAll('input[type=file]')].map(x=>({file:x.getAttribute('data-drop-file-input'),gallery:x.getAttribute('data-drop-gallery-input'),disabled:x.disabled,accept:x.accept})),html:document.documentElement.outerHTML.includes('data-drop-queue-item')})); console.error('IMAGE_CARD_DEBUG',JSON.stringify(debug,null,2)); throw error; }
 ...
 const imageCommentValue=await sendPage.evaluate(()=>[...document.querySelectorAll('textarea')].find(el=>el.placeholder?.includes('képen látható')||el.placeholder?.includes('Megjegyzés azonnal'))?.value||'');
 pass('image-voice-evolving-result-not-duplicated',imageCommentValue.includes('teszt hangos levélüzenet')&&!imageCommentValue.includes('teszt teszt'),imageCommentValue);
