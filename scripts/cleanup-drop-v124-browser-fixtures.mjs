...
async function main() {
  const users = await client.from('dimpro_users').select('id,email').ilike('email', 'drop-v124-browser-%');
  if (users.error) throw users.error;
  const userRows = users.data || [];
  for (const row of userRows) {
    assert.match(row.email || '', /^drop-v124-browser-.*@example\.invalid$/);
  }
  const userIds = userRows.map((row) => row.id);
  const orphanOrganizations = await client.from('dimpro_organizations').select('id,display_name,legal_name').or('display_name.ilike.DIMPRO V124 Browser Org %,legal_name.ilike.DIMPRO V124 Browser Org %');
  if (orphanOrganizations.error) throw orphanOrganizations.error;
  const orphanOrganizationIds = (orphanOrganizations.data || []).filter((row) => /DIMPRO V124 Browser Org/.test(`${row.display_name || ''} ${row.legal_name || ''}`)).map((row) => row.id);
  if (!userIds.length) {
    if (orphanOrganizationIds.length) { const deleted = await client.from('dimpro_organizations').delete().in('id', orphanOrganizationIds); if (deleted.error) throw deleted.error; }
    console.log(JSON.stringify({ ok: true, deletedUsers: 0, deletedOrganizations: orphanOrganizationIds.length, deletedEntitlements: 0, deletedLicenses: 0 }));
    return;
  }
  ...
  const removableOrganizationIds = [...new Set([...organizationIds, ...orphanOrganizationIds])];
  if (removableOrganizationIds.length) {
    const organizationRows = await client.from('dimpro_organizations').select('id,display_name,legal_name').in('id', removableOrganizationIds);
    ...
    const removable = (organizationRows.data || []).filter((row) => {
      ...
      return /DIMPRO V124 Browser Org/.test(name);
    }).map((row) => row.id);
    if (removable.length) {
  ...
  console.log(JSON.stringify({
    ...
    deletedOrganizations: removableOrganizationIds.length,
