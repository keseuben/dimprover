...
check("Matching lockfile uses Turbopack-safe hardlink tree",()=>assert.ok(helper.includes('cp -al "$DEPENDENCY_SOURCE_ROOT/node_modules" "$TARGET/node_modules"')&&!helper.includes('ln -s "$OPERATOR_ROOT/node_modules"')));
check("Symlink dependency source is rejected",()=>assert.ok(helper.includes('[[ ! -L "$CANDIDATE_ROOT/node_modules" ]]')&&helper.includes("benjadmin-operator-ui-v2")));
...
check("Retention keeps newest builds for active worktrees",()=>assert.ok(retention.includes("keepNewestPerWorktree")&&retention.includes("!retiredWorktree && newest.has")));
...
check("Retention never auto deletes backups",()=>assert.ok(retention.includes("Backup és artifact könyvtárak")&&retention.includes("soha nem törlődnek automatikusan")));
