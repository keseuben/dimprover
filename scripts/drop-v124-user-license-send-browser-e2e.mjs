...
const PORT=Number(process.env.DROP_V124_BROWSER_PORT||3120);
...
const fullName=`DROP 1.2.4 Browser User ${stamp}`;
const email=`drop-v124-browser-${stamp}@example.invalid`;
const phone='+36 30 987 6543';
const organizationName=`DIMPRO V124 Browser Org ${stamp}`;
...
try{
 ...
 const r=await page.goto(`${LICENSE}/drive/drop/public-workflows?v124=1`,{waitUntil:'networkidle2',timeout:120000});
 pass('admin-http',r?.status()===200,String(r?.status()));
 await page.waitForFunction(()=>(document.body.textContent||'').includes('DROP 1.2.4')&&(document.body.textContent||'').includes('Új Send entitlement'),{timeout:30000});
 pass('v124-admin-visible',true);
 ...
 const sr=await sendPage.goto(`${DROP}/send?v124-login=1`,{waitUntil:'networkidle2',timeout:120000});
 ...
 console.log(JSON.stringify({ok:true,version:'DROP 1.2.4',checks:checks.length,names:checks},null,2));
} finally {
