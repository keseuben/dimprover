...
set -Eeuo pipefail
ROOT="/root/dimprover"
BACKUP_REF="$ROOT/.work_drop_v091_release_activation_backup"
if [[ ! -f "$BACKUP_REF" ]]; then echo "Hiányzó DROP 0.9.1 rollback-hivatkozás: $BACKUP_REF" >&2; exit 2; fi
...
if [[ ! -f "$ENV_BACKUP" || ! -f "$POINTER_BACKUP" ]]; then echo "A DROP 0.9.1 rollback-mentés hiányos: $BACKUP_DIR" >&2; exit 4; fi
...
cp -a "$ROOT/.env.local" "$ROOT/.env.local.before-v091-rollback-$(date +%Y%m%d_%H%M%S)"
...
echo "DROP 0.9.1 rollback kész. Aktív release: $PREVIOUS_RELEASE; build: $ACTIVE_BUILD"
