...
const screenshotDir = process.env.DIMPRO_TEST_SCREENSHOT_DIR || '/tmp/dimpro_v08447_revision_export_screenshots';
const downloadDir = process.env.DIMPRO_TEST_DOWNLOAD_DIR || '/tmp/dimpro_v08447_revision_export_downloads';
const fixtureDir = process.env.DIMPRO_TEST_FIXTURE_DIR || '/tmp/dimpro_v08447_revision_export_fixtures';
...
async function waitForDownload(previousNames, predicate, timeout = 30000) {
  const startedAt = Date.now();
  while (Date.now() - startedAt < timeout) {
    const candidate = fs.readdirSync(downloadDir).find((name) => !previousNames.includes(name) && !name.endsWith('.crdownload') && predicate(name));
    if (candidate) {
      const fullPath = path.join(downloadDir, candidate);
      const firstSize = fs.statSync(fullPath).size;
      await sleep(350);
      if (fs.existsSync(fullPath) && fs.statSync(fullPath).size === firstSize && firstSize > 0) return fullPath;
    }
    await sleep(150);
  }
  throw new Error(`A letöltött fájl nem jelent meg ${timeout} ms alatt. Tartalom: ${fs.readdirSync(downloadDir).join(', ')}`);
}
...
async function createProjectAndSurvey(page) {
  ...
  await page.type('input[placeholder="Projekt neve *"]', 'V08447 revíziócsomag regresszió');
  await page.type('input[placeholder="Projektkód"]', 'V08447-EXPORT');
  await page.type('input[placeholder="Település / helyszín"]', '7100 Szekszárd');
  await clickText(page, 'Projekt létrehozása');
  await waitText(page, 'V08447 revíziócsomag regresszió');
  ...
  await nameInput.type('V08447 revíziócsomag és megosztás');
...
(async () => {
  ...
  const seeded = await page.evaluate(() => {
    ...
    for (const item of revisions) {
      ...
      item.document.versionGroupId = 'v08447-main-group';
  ...
  await page.waitForSelector('[data-plan-version-export-panel][data-plan-version-publish-ready="true"]', { timeout: 30000 });
  const exportPanel = await page.evaluate(() => ({
    text: document.querySelector('[data-plan-version-export-panel]')?.textContent || '',
    packageDisabled: document.querySelector('[data-plan-version-export-package]')?.disabled,
    pdfDisabled: document.querySelector('[data-plan-version-export-pdf]')?.disabled,
    jsonDisabled: document.querySelector('[data-plan-version-export-json]')?.disabled,
    serverDisabled: document.querySelector('[data-plan-version-server-publish-disabled]')?.disabled,
  }));
  assert(exportPanel.text.includes('3') && exportPanel.text.includes('2') && exportPanel.text.includes('8') && exportPanel.packageDisabled === false && exportPanel.pdfDisabled === false && exportPanel.jsonDisabled === false && exportPanel.serverDisabled === true, `Az exportpanel hibás: ${JSON.stringify(exportPanel)}`);
  pass('A revíziócsomag panel három dokumentumot, két összehasonlítást, nyolc csomagfájlt és inaktív szerveres publikálást mutat');

  let beforeDownloads = fs.readdirSync(downloadDir);
  await page.click('[data-plan-version-export-json]');
  const manifestFile = await waitForDownload(beforeDownloads, (name) => name.endsWith('_manifest.json'));
  const manifest = JSON.parse(fs.readFileSync(manifestFile, 'utf8'));
  assert(manifest.schema === 'dimpro.plan-revision-package.v1' && manifest.source?.productVersion === 'v0.8.4.4.7' && manifest.documents?.length === 3 && manifest.comparisons?.length === 2 && manifest.snapshots?.length === 2, `A letöltött manifest alapadatai hibásak: ${JSON.stringify({ schema: manifest.schema, source: manifest.source, docs: manifest.documents?.length, comparisons: manifest.comparisons?.length, snapshots: manifest.snapshots?.length })}`);
  assert(!fs.readFileSync(manifestFile, 'utf8').includes('data:application/pdf') && manifest.sharedRevisionEnvelope?.publishState === 'localDraft' && manifest.sharedRevisionEnvelope?.transport?.status === 'notConfigured', 'A manifest PDF dataUrl-t tartalmaz vagy a megosztási envelope hibás.');
  pass('A böngészőből letöltött JSON manifest teljes, determinisztikus és nem tartalmaz beágyazott PDF-adatot');

  beforeDownloads = fs.readdirSync(downloadDir);
  await page.click('[data-plan-version-export-pdf]');
  const summaryPdfFile = await waitForDownload(beforeDownloads, (name) => name.endsWith('_osszefoglalo.pdf'));
  const summaryPdfBytes = fs.readFileSync(summaryPdfFile);
  const summaryPdf = await PDFDocument.load(summaryPdfBytes);
  assert(summaryPdfBytes.subarray(0, 4).toString() === '%PDF' && summaryPdf.getPageCount() >= 1 && summaryPdfBytes.length > 2000, 'A letöltött PDF revíziós összefoglaló hibás vagy üres.');
  pass('A böngészőből letöltött revíziós PDF érvényes és nyomtatható dokumentum');

  beforeDownloads = fs.readdirSync(downloadDir);
  await page.click('[data-plan-version-export-package]');
  const zipFile = await waitForDownload(beforeDownloads, (name) => name.endsWith('.zip'), 45000);
  const zipBuffer = fs.readFileSync(zipFile);
  const zip = await JSZip.loadAsync(zipBuffer);
  const zipNames = Object.keys(zip.files).sort();
  for (const required of ['README.txt', 'checksums.json', 'manifest.json', 'osszefoglalo.pdf', 'shared-revision-envelope.json', 'valtozasok.csv']) assert(zipNames.includes(required), `A ZIP-ből hiányzik: ${required}`);
  const snapshotFiles = zipNames.filter((name) => name.startsWith('snapshots/') && name.endsWith('.json'));
  const zipManifest = JSON.parse(await zip.file('manifest.json').async('string'));
  const envelope = JSON.parse(await zip.file('shared-revision-envelope.json').async('string'));
  const checksums = JSON.parse(await zip.file('checksums.json').async('string'));
  const csv = await zip.file('valtozasok.csv').async('string');
  const zipPdfBytes = Buffer.from(await zip.file('osszefoglalo.pdf').async('uint8array'));
  const zipPdf = await PDFDocument.load(zipPdfBytes);
  assert(snapshotFiles.length === 2
    && zipManifest.schema === 'dimpro.plan-revision-package.v1'
    && zipManifest.sharedRevisionEnvelope?.revisionId === envelope.revisionId
    && envelope.revisionId === manifest.sharedRevisionEnvelope.revisionId
    && zipManifest.documents?.length === manifest.documents.length
    && zipManifest.comparisons?.length === manifest.comparisons.length
    && zipManifest.snapshots?.length === manifest.snapshots.length
    && checksums.algorithm === 'fnv1a32-canonical-json'
    && Object.keys(checksums.snapshots || {}).length === 2
    && csv.includes('Összehasonlítás ID')
    && zipPdf.getPageCount() >= 1, `A ZIP tartalma vagy konzisztenciája hibás: ${JSON.stringify({ zipNames, snapshotFiles, checksums })}`);
  pass('A böngészőből letöltött ZIP manifestet, PDF-et, CSV-t, envelope-ot, checksumot és két deduplikált snapshotot tartalmaz');

  await page.waitForSelector('[data-plan-version-export-message][data-plan-version-export-state="done"]', { timeout: 30000 });
  await page.screenshot({ path: path.join(screenshotDir, 'revision_export_desktop.png'), fullPage: false });
  ...
  await page.screenshot({ path: path.join(screenshotDir, 'revision_export_1194x834.png'), fullPage: false });
  await setViewport(page, 834, 1194);
  await page.screenshot({ path: path.join(screenshotDir, 'revision_export_834x1194.png'), fullPage: false });
  assert(fs.statSync(path.join(screenshotDir, 'revision_export_1194x834.png')).size > 10000 && fs.statSync(path.join(screenshotDir, 'revision_export_834x1194.png')).size > 10000, 'A revíziócsomag tablet screenshot-regresszió hiányos.');
  pass('A verziógráf és revíziócsomag fekvő és álló tableten overflow nélkül megjelenik');
  ...
  pass('A teljes revíziócsomag-, gráf- és történeti rollback E2E konzol- és oldalhiba nélkül futott');

  console.log(`DIMPRO Felmérő v0.8.4.4.7 revíziócsomag E2E: ${testCount}/${testCount} sikeres`);
  tests.forEach((message, index) => console.log(`${index + 1}. ${message}`));
  console.log(`Letöltések: ${downloadDir}`);
