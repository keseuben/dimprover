...
check("Négy feature flag külön létezik", ["BENJADMIN_TERMINAL_HUB_ENABLED", "BENJADMIN_LIVE_WORKSPACE_ENABLED", "BENJADMIN_TERMINAL_EXECUTION_ENABLED", "BENJADMIN_DESKTOP_BRIDGE_ENABLED"].every((name) => config.includes(name)));
check("Execution és Desktop Bridge alapból false", config.includes('flag("BENJADMIN_TERMINAL_EXECUTION_ENABLED", false)') && config.includes('flag("BENJADMIN_DESKTOP_BRIDGE_ENABLED", false)'));
...
check("Terminál sanitization a meglévő secret-scannert használja", dataPolicy.includes("scanSensitiveText") && dataPolicy.includes("REDACTED_SENSITIVE_TERMINAL_DATA"));
