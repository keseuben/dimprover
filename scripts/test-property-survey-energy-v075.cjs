...
const downloadDir = process.env.DIMPRO_TEST_DOWNLOAD_DIR || '/tmp/dimpro_energy_v075_downloads';
...
async function createProjectAndSurvey(page) {
  ...
  await page.type('input[placeholder="Projekt neve *"]', 'V075 energetikai teszt');
  await page.type('input[placeholder="Projektkód"]', 'V075');
  ...
  await waitText(page, 'V075 energetikai teszt');
  ...
  await inputs[0].type('V075 energetikai mintafelmérés');
...
(async () => {
  ...
  assert(await page.$('[data-energy-tab="demand"]'), 'A Zónaterhelés fül hiányzik.');
  ...
  pass('Nyolcfüles energetikai munkatér zóna-, nyílászáró-, terhelés- és U-érték nézettel megnyílik');
  ...
  assert((await page.$eval('[data-energy-demand-engine-card]', (element) => element.textContent)).includes('Zónaterhelési motor 0.7.5'), 'A zónaterhelési motor státuszkártyája hiányzik.');
  assert((await page.$eval('[data-energy-demand-engine-card]', (element) => element.textContent)).includes('nincs bekapcsolva'), 'A régi projekt kompatibilis, kikapcsolt terhelési állapota hiányzik.');
  pass('Állapotlap geometria-, zóna-, U-, nyílászáró- és zónaterhelési motorverziót mutat');
  ...
  await page.waitForSelector('[data-energy-openings-panel]', { timeout: 20000 });
  ...
  await setNativeValue(page, `[data-unheated-temperature-source-reference="${unheatedId}"]`, 'UNHEATED-TEMP-REF');
  ...
  assert(storedZones.unheatedSpaces[0].type === 'garage' && storedZones.unheatedSpaces[0].designTemperatureC === 8 && storedZones.unheatedSpaces[0].temperatureSourceReference === 'UNHEATED-TEMP-REF', 'A fűtetlen tér beállításai vagy forráshivatkozása nem mentődött.');
  pass('Zónák, helyiséghozzárendelések és dokumentált fűtetlen tér beállítások mentődnek');

  await page.click('[data-energy-tab="openings"]');
  await page.click('[data-energy-opening-section="bridges"]');
  await page.waitForSelector(`[data-bridge-zone="${linearBridgeId}"]`);
  await page.select(`[data-bridge-zone="${linearBridgeId}"]`, zoneIds[0]);
  await page.select(`[data-bridge-zone="${pointBridgeId}"]`, secondZoneId);
  await page.waitForFunction((firstId, secondId, firstZone, secondZone) => document.querySelector(`[data-bridge-zone="${firstId}"]`)?.value === firstZone && document.querySelector(`[data-bridge-zone="${secondId}"]`)?.value === secondZone, {}, linearBridgeId, pointBridgeId, zoneIds[0], secondZoneId);
  await page.waitForFunction((firstId, secondId, firstZone, secondZone) => {
    const workspace=JSON.parse(localStorage.getItem('dimpro-property-survey-workspace-v1'));
    const survey=workspace.surveys.find((item)=>item.id===workspace.activeSurveyId);
    const bridges=survey.draft.energyOpeningWorkspace.thermalBridges;
    return bridges.find((bridge)=>bridge.id===firstId)?.zoneId===firstZone && bridges.find((bridge)=>bridge.id===secondId)?.zoneId===secondZone;
  }, { timeout:10000 }, linearBridgeId, pointBridgeId, zoneIds[0], secondZoneId);
  pass('A külön lineáris és pontszerű hőhidak explicit energetikai zónához rendelhetők és mentődnek');

  const structuralFixture = await page.evaluate(() => {
    const key = 'dimpro-property-survey-workspace-v1';
    const workspace = JSON.parse(localStorage.getItem(key));
    const survey = workspace.surveys.find((item) => item.id === workspace.activeSurveyId);
    const draft = survey.draft;
    const now = new Date().toISOString();
    const wallAssembly = draft.assemblies.find((assembly) => assembly.category === 'wall');
    if (!wallAssembly) throw new Error('A falrétegrend hiányzik a történeti energetikai fixture-höz.');
    const createDeclared = ({ id, category, name, u, heatFlowDirection, boundaryMode, requirementType }) => ({
      id,
      category,
      name,
      heatFlowDirection,
      ...
      complexity:'homogeneous',
      requirementType,
      customRequirementMaximumUValueWm2K:undefined,
      surfaceResistanceMode:'ruleSetDefault',
      customInsideSurfaceResistanceM2KPerW:undefined,
      customOutsideSurfaceResistanceM2KPerW:undefined,
      declaredUValueWm2K:String(u),
      declaredUValueSource:`${id}-SOURCE`,
      corrections:JSON.parse(JSON.stringify(wallAssembly.corrections)),
      layers:[],
      note:'V075 böngészőteszt deklarált rétegrend.',
      createdAt:now,
      updatedAt:now,
    });
    const floorAssembly = createDeclared({ id:'assembly-v075-floor', category:'floor', name:'V075 talajon fekvő padló', u:0.25, heatFlowDirection:'downward', boundaryMode:'groundEquivalentRequired', requirementType:'groundFloor' });
    const ceilingAssembly = createDeclared({ id:'assembly-v075-ceiling', category:'ceiling', name:'V075 padlásfödém', u:0.16, heatFlowDirection:'upward', boundaryMode:'externalAir', requirementType:'atticFloor' });
    draft.assemblies = [...draft.assemblies.filter((assembly) => !['assembly-v075-floor','assembly-v075-ceiling'].includes(assembly.id)), floorAssembly, ceilingAssembly];
    draft.rooms = draft.rooms.map((room) => room.heated ? { ...room, floorAssemblyId:floorAssembly.id, ceilingAssemblyId:ceilingAssembly.id } : room);
    draft.wallSegments = draft.wallSegments.map((segment) => ({ ...segment, assemblyId:wallAssembly.id, updatedAt:now }));
    survey.draft = draft;
    localStorage.setItem(key, JSON.stringify(workspace));
    return { wallAssemblyId:wallAssembly.id, floorAssemblyId:floorAssembly.id, ceilingAssemblyId:ceilingAssembly.id, heatedRoomCount:draft.rooms.filter((room)=>room.heated).length, wallCount:draft.wallSegments.length };
  });
  assert(structuralFixture.heatedRoomCount === 6 && structuralFixture.wallCount > 0, `A terhelési szerkezeti fixture hibás: ${JSON.stringify(structuralFixture)}`);
  await page.reload({ waitUntil:'domcontentloaded' });
  await page.waitForSelector('[data-survey-step="energy"]');
  pass('Minden fűtött helyiség és falszakasz explicit fal-, padló- és födémrétegrendet kapott');

  await page.click('[data-survey-step="energy"]');
  await page.click('[data-energy-tab="demand"]');
  await page.waitForSelector('[data-energy-demand-panel]');
  assert((await page.$eval('[data-energy-demand-panel]', (element) => element.getAttribute('data-energy-demand-enabled'))) === 'false', 'A régi projekt terhelési rétege nem kikapcsolt állapotból indul.');
  await page.click('[data-demand-enabled]');
  await page.waitForFunction(() => document.querySelector('[data-energy-demand-panel]')?.getAttribute('data-energy-demand-enabled') === 'true');
  await setDecimalField(page, '[data-demand-external-temperature]', '-13');
  await setNativeValue(page, '[data-demand-external-source]', 'OUTDOOR-DESIGN-REF');
  await setDecimalField(page, '[data-demand-air-capacity]', '0,34');
  await setNativeValue(page, '[data-demand-air-capacity-source]', 'AIR-CAPACITY-REF');
  pass('Dokumentált külső méretezési hőmérséklet és levegő-hőkapacitás rögzíthető');

  await page.click('[data-energy-demand-section="zones"]');
  for (const zoneId of zoneIds) {
    await setDecimalField(page, `[data-demand-air-change="${zoneId}"]`, '0,5');
    await setDecimalField(page, `[data-demand-heat-recovery="${zoneId}"]`, '0,2');
    await setNativeValue(page, `[data-demand-ventilation-source="${zoneId}"]`, `VENT-${zoneId}`);
  }
  try {
    await page.waitForFunction(() => document.querySelector('[data-energy-demand-panel]')?.getAttribute('data-energy-demand-valid') === 'true', { timeout:15000 });
  } catch (error) {
    const diagnosis = await page.evaluate(() => ({
      enabled:document.querySelector('[data-energy-demand-panel]')?.getAttribute('data-energy-demand-enabled'),
      valid:document.querySelector('[data-energy-demand-panel]')?.getAttribute('data-energy-demand-valid'),
      codes:[...document.querySelectorAll('[data-demand-validation-code]')].map((element)=>element.getAttribute('data-demand-validation-code')),
      messages:[...document.querySelectorAll('[data-demand-validation-code]')].map((element)=>element.textContent),
      cards:[...document.querySelectorAll('[data-demand-zone-card]')].map((element)=>element.textContent),
    }));
    throw new Error(`A zónaterhelés nem vált számíthatóvá: ${JSON.stringify(diagnosis)} / ${error.message}`);
  }
  const demandWithoutSystems = await page.evaluate(() => ({
    valid:document.querySelector('[data-energy-demand-panel]')?.getAttribute('data-energy-demand-valid'),
    zones:document.querySelectorAll('[data-demand-zone-card]').length,
    missing:[...document.querySelectorAll('[data-demand-system-status]')].filter((element)=>element.getAttribute('data-demand-system-status')==='missing').length,
    texts:[...document.querySelectorAll('[data-demand-zone-card]')].map((element)=>element.textContent),
  }));
  assert(demandWithoutSystems.valid === 'true' && demandWithoutSystems.zones === 2 && demandWithoutSystems.missing === 2, `A rendszer nélküli, de számítható zónaterhelés hibás: ${JSON.stringify(demandWithoutSystems)}`);
  assert(demandWithoutSystems.texts.every((text) => text.includes('kW') && text.includes('W/m²')), 'A zónánkénti teljesítmény vagy fajlagos eredmény hiányzik.');
  pass('A zónaterhelés rendszer nélkül is számítható, miközben a rendszerhiány figyelmeztetés marad');

  await page.click('[data-energy-demand-section="systems"]');
  await page.click('[data-add-energy-system]');
  const systemCard = await page.waitForSelector('[data-energy-system-card]');
  const energySystemId = await systemCard.evaluate((element) => element.getAttribute('data-energy-system-card'));
  assert(energySystemId, 'Az energetikai rendszer azonosítója hiányzik.');
  await setNativeValue(page, `[data-system-name="${energySystemId}"]`, 'V075 központi hőszivattyú');
  await page.select(`[data-system-service="${energySystemId}"]`, 'heating');
  await page.select(`[data-system-type="${energySystemId}"]`, 'heatPump');
  await setDecimalField(page, `[data-system-capacity="${energySystemId}"]`, '1');
  await setNativeValue(page, `[data-system-source="${energySystemId}"]`, 'HEATPUMP-DOP-001');
  for (const zoneId of zoneIds) {
    await page.click(`[data-system-zone="${energySystemId}:${zoneId}"]`);
    await page.waitForSelector(`[data-system-zone-capacity="${energySystemId}:${zoneId}"]`);
    await setDecimalField(page, `[data-system-zone-capacity="${energySystemId}:${zoneId}"]`, '0,1');
  }
  await page.click('[data-energy-demand-section="results"]');
  try {
    await page.waitForFunction(() => [...document.querySelectorAll('[data-demand-system-status]')].some((element) => element.getAttribute('data-demand-system-status') === 'insufficient'), { timeout:15000 });
  } catch (error) {
    const diagnosis=await page.evaluate(() => ({
      statuses:[...document.querySelectorAll('[data-demand-system-status]')].map((element)=>({status:element.getAttribute('data-demand-system-status'),text:element.textContent})),
      systemCards:[...document.querySelectorAll('[data-energy-system-card]')].map((element)=>element.textContent),
      validation:[...document.querySelectorAll('[data-demand-validation-code]')].map((element)=>({code:element.getAttribute('data-demand-validation-code'),text:element.textContent})),
      workspace:(()=>{const w=JSON.parse(localStorage.getItem('dimpro-property-survey-workspace-v1')); const s=w.surveys.find((item)=>item.id===w.activeSurveyId); return s.draft.energyDemandWorkspace;})(),
    }));
    throw new Error(`Az elégtelen kapacitásállapot nem jelent meg: ${JSON.stringify(diagnosis)} / ${error.message}`);
  }
  pass('Az elégtelen zónakapacitást a rendszer külön figyelmeztetésként kimutatja');

  await page.click('[data-energy-demand-section="systems"]');
  await setDecimalField(page, `[data-system-capacity="${energySystemId}"]`, '50');
  for (const zoneId of zoneIds) await setDecimalField(page, `[data-system-zone-capacity="${energySystemId}:${zoneId}"]`, '20');
  await page.click('[data-energy-demand-section="results"]');
  await page.waitForFunction(() => {
    const statuses=[...document.querySelectorAll('[data-demand-system-status]')].map((element)=>element.getAttribute('data-demand-system-status'));
    return statuses.length === 2 && statuses.every((status)=>status === 'sufficient') && document.querySelector('[data-energy-demand-panel]')?.getAttribute('data-energy-demand-valid') === 'true';
  }, { timeout:15000 });
  pass('A megfelelő névleges és zónánként kiosztott kapacitás mindkét zónánál megfelelő állapotot ad');

  await page.waitForSelector('[data-demand-results]');
  const demandResultState = await page.evaluate(() => ({
    text:document.querySelector('[data-demand-results]')?.textContent,
    zones:document.querySelectorAll('[data-demand-result-zone]').length,
    components:document.querySelectorAll('[data-demand-component]').length,
    statuses:[...document.querySelectorAll('[data-demand-system-status]')].map((element)=>element.getAttribute('data-demand-system-status')),
  }));
  assert(demandResultState.zones === 2 && demandResultState.components >= 10, `A részletes terhelési eredmény hiányos: ${JSON.stringify(demandResultState)}`);
  assert(demandResultState.text.includes('H transzmisszió') && demandResultState.text.includes('H szellőzés') && demandResultState.text.includes('Fűtési igény'), 'A terhelési főmutatók hiányoznak.');
  pass('Zónánkénti fal-, padló-, födém-, nyílászáró-, hőhíd- és szellőzési komponensek megjelennek');

  await page.click('[data-energy-demand-section="trace"]');
  const demandTraceState = await page.evaluate(() => ({ count:document.querySelectorAll('[data-demand-trace-rule]').length, rules:[...document.querySelectorAll('[data-demand-trace-rule]')].map((element)=>element.getAttribute('data-demand-trace-rule')) }));
  assert(demandTraceState.count >= 12, `A zónaterhelési nyomvonal túl rövid: ${demandTraceState.count}`);
  for (const rule of ['DEMAND-WALL-TRANSMISSION-002','DEMAND-LOWER-BOUNDARY-003','DEMAND-UPPER-BOUNDARY-004','DEMAND-VENTILATION-008','DEMAND-DESIGN-HEATING-POWER-010']) assert(demandTraceState.rules.includes(rule), `Hiányzó terhelési szabály: ${rule}`);
  pass('A zónaterhelési auditnyomvonal minden fő képletet és forrást tartalmaz');

  await new Promise((resolve)=>setTimeout(resolve,900));
  const storedDemand = await page.evaluate(() => {
    const workspace=JSON.parse(localStorage.getItem('dimpro-property-survey-workspace-v1'));
    const survey=workspace.surveys.find((item)=>item.id===workspace.activeSurveyId);
    return survey.draft.energyDemandWorkspace;
  });
  assert(storedDemand.schemaVersion === 1 && storedDemand.enabled === true && storedDemand.systems.length === 1 && Object.keys(storedDemand.zoneSettings).length === 2, 'A zónaterhelési munkatér vagy rendszerkapcsolatok nem mentődtek.');
  assert(storedDemand.systems[0].servedZoneIds.length === 2 && Object.values(storedDemand.systems[0].zoneCapacityAllocationsKw).every((value)=>value === 20), 'A zónánkénti rendszerkapacitás-kiosztás nem mentődött.');
  pass('A terhelési beállítások, rendszer és zónakapacitás-kiosztás projektállapotba mentődik');
  ...
  assert(dimpro.schema === 'dimpro.property-survey.v0.8.4.3', `Hibás .dimpro v0.8.4.3 séma: ${dimpro.schema}`);
  assert(dimpro.calculated?.winWattFieldMap?.schema === 'dimpro.winwatt-field-map.v0.8.3', 'A WinWatt mezőtérkép hiányzik a .dimpro fájlból.');
  ...
  assert(dimpro.calculated.energyDemand.schema === 'dimpro.energy-demand-set.v0.7.5', 'A zónaterhelési eredmény hiányzik a .dimpro fájlból.');
  assert(dimpro.calculated.energyDemand.enabled === true && dimpro.calculated.energyDemand.valid === true && dimpro.calculated.energyDemand.zones.length === 2 && dimpro.calculated.energyDemand.systems.length === 1 && dimpro.calculated.energyDemand.trace.length >= 12, 'A zónaterhelési eredmény, rendszer vagy auditnyomvonal hiányos a .dimpro fájlban.');
  assert(dimpro.calculated.energyRenewables.schema === 'dimpro.energy-renewable-sizing.v0.8.0' && dimpro.calculated.energyRenewables.enabled === false, 'A kikapcsolt megújuló eredmény hiányzik a .dimpro fájlból.');
  assert(dimpro.calculated.energyRenovationComparison.schema === 'dimpro.energy-renovation-comparison.v0.8.2', 'A változat-összehasonlító eredmény hiányzik a .dimpro fájlból.');
  ...
  pass('.dimpro v0.8.2 export tartalmazza a geometriát, zónákat, rétegrendeket, nyílászárókat, hőhidakat, terhelést és rendszereket');
  ...
  assert(winWatt.schema === 'dimpro.winwatt-compatible.v0.8.4', `Hibás WinWatt séma: ${winWatt.schema}`);
  assert(winWatt.winWattFieldMap?.schema === 'dimpro.winwatt-field-map.v0.8.3', 'A WinWatt mezőtérkép hiányzik a WinWatt-előkészítő JSON-ból.');
  assert(winWatt.trialPackageSchema === 'dimpro.winwatt-trial-package.v0.8.4', 'A próbaátadási csomagséma hiányzik a WinWatt-előkészítő JSON-ból.');
  ...
  assert(winWatt.demandWorkspace.enabled === true && winWatt.zoneDesignLoads.length === 2 && winWatt.energySystems.length === 1 && winWatt.demandComponents.length >= 10, 'A WinWatt zónaterhelési vagy rendszerblokk hiányos.');
  assert(winWatt.demandTotals.designHeatingPowerKw > 0 && winWatt.demandTotals.sufficientZoneCount === 2 && winWatt.demandTrace.length >= 12, 'A WinWatt terhelési összesítő vagy auditnyomvonal hibás.');
  assert(winWatt.renewableSizing.schema === 'dimpro.energy-renewable-sizing.v0.8.0' && winWatt.renewableSizing.enabled === false, 'A WinWatt-előkészítő megújuló blokkja hiányzik.');
  assert(winWatt.renovationComparison.schema === 'dimpro.energy-renovation-comparison.v0.8.2', 'A WinWatt-előkészítő változat-összehasonlító blokkja hiányzik.');
  assert(winWatt.transferWorkbookSchema === 'dimpro.winwatt-transfer.v0.8.4', 'Az Excel átadási séma hiányzik a WinWatt-előkészítő JSON-ból.');
  pass('WinWatt-előkészítő zónákat, nyílászárókat, hőhidakat, zónaterhelést és rendszerkapcsolatokat tartalmaz');
  ...
  const demandPdfPage = pdfTexts.find((text) => normalizePdfText(text).includes('ZONANKENTI MERETEZESI FUTESI TERHELES'));
  const systemsPdfPage = pdfTexts.find((text) => normalizePdfText(text).includes('GEPESZETI RENDSZERKAPCSOLATOK'));
  ...
  const normalizedDemandPdfPage = normalizePdfText(demandPdfPage || '');
  const normalizedSystemsPdfPage = normalizePdfText(systemsPdfPage || '');
  ...
  assert(normalizedDemandPdfPage.includes('Htranszmisszio') && normalizedDemandPdfPage.includes('Hszellozes') && normalizedDemandPdfPage.includes('kW'), `A PDF zónaterhelési oldala hiányos: ${normalizedDemandPdfPage.slice(0,900)}`);
  assert(normalizedSystemsPdfPage.includes('V075 kozponti hoszivattyu') && normalizedSystemsPdfPage.includes('50.000 kW') && normalizedSystemsPdfPage.includes('20.000 kW'), `A PDF rendszerkapcsolati oldala hiányos: ${normalizedSystemsPdfPage.slice(0,900)}`);
  pass('Vektoros PDF külön zóna-, nyílászáró-, hőhíd-, terhelés-, rendszer- és U-érték oldalakat tartalmaz');
  ...
  await page.evaluate(() => {
    const key = 'dimpro-property-survey-workspace-v1';
    const workspace = JSON.parse(localStorage.getItem(key));
    for (const survey of workspace.surveys) { delete survey.draft.energyProjectSettings; delete survey.draft.energyZoneWorkspace; delete survey.draft.energyOpeningWorkspace; delete survey.draft.energyDemandWorkspace; delete survey.draft.energyFieldWorkflow; delete survey.draft.energyRenovationWorkspace; delete survey.draft.energyRenewableWorkspace; delete survey.draft.materialWorkspace; }
    localStorage.setItem(key, JSON.stringify(workspace));
  });
  await page.reload({ waitUntil: 'domcontentloaded' });
  ...
  const migrated = await page.evaluate(() => {
    const workspace = JSON.parse(localStorage.getItem('dimpro-property-survey-workspace-v1'));
    const survey = workspace.surveys.find((item) => item.id === workspace.activeSurveyId);
    return { energySchema: survey.draft.energyProjectSettings?.schemaVersion, zoneSchema: survey.draft.energyZoneWorkspace?.schemaVersion, zoneCount: survey.draft.energyZoneWorkspace?.zones?.length, assignedRooms: Object.keys(survey.draft.energyZoneWorkspace?.roomAssignments || {}).length, openingSchema: survey.draft.energyOpeningWorkspace?.schemaVersion, openingDetailCount: Object.keys(survey.draft.energyOpeningWorkspace?.openingDetails || {}).length, migratedOpeningMode: Object.values(survey.draft.energyOpeningWorkspace?.openingDetails || {})[0]?.calculationMode, demandSchema: survey.draft.energyDemandWorkspace?.schemaVersion, demandEnabled: survey.draft.energyDemandWorkspace?.enabled, demandZoneSettings: Object.keys(survey.draft.energyDemandWorkspace?.zoneSettings || {}).length, demandSystems: survey.draft.energyDemandWorkspace?.systems?.length, fieldSchema: survey.draft.energyFieldWorkflow?.schemaVersion, fieldMode: survey.draft.energyFieldWorkflow?.mode, renovationSchema: survey.draft.energyRenovationWorkspace?.schemaVersion, scenarioCount: survey.draft.energyRenovationWorkspace?.scenarios?.length, renewableSchema: survey.draft.energyRenewableWorkspace?.schemaVersion, renewableEnabled: survey.draft.energyRenewableWorkspace?.enabled, materialSchema: survey.draft.materialWorkspace?.schemaVersion, materialProjectId: survey.draft.materialWorkspace?.projectCatalog?.projectId, projectId: survey.projectId };
  });
  assert(migrated.energySchema === 1 && migrated.zoneSchema === 1 && migrated.zoneCount === 1 && migrated.assignedRooms === 6 && migrated.openingSchema === 1 && migrated.openingDetailCount === 1 && migrated.migratedOpeningMode === 'declared' && migrated.demandSchema === 1 && migrated.demandEnabled === false && migrated.demandZoneSettings === 1 && migrated.demandSystems === 0 && migrated.fieldSchema === 1 && migrated.fieldMode === 'field' && migrated.renovationSchema === 1 && migrated.scenarioCount === 2 && migrated.renewableSchema === 1 && migrated.renewableEnabled === false && migrated.materialSchema === 1 && migrated.materialProjectId === migrated.projectId, `Régi projekt migrációja hibás: ${JSON.stringify(migrated)}`);
  pass('v0.6.x projekt energetikai, zóna-, nyílászáró-, kikapcsolt terhelési, felújítási, megújuló és anyagtér-migrációja működik');
  await page.click('[data-energy-mode="expert"]');
  await page.waitForSelector('[data-survey-step="energy"]');
  ...
  assert(uiTests.length >= 42, `Kevesebb mint 42 UI-teszt futott: ${uiTests.length}`);
  ...
  console.log(JSON.stringify({ ok:true, uiTestCount:uiTests.length, uiTests, geometryState, traceState, catalogResponsive, stored, dimproFile, dimproSchema:dimpro.schema, geometrySchema:dimpro.calculated.energyGeometry.schema, assemblySchema:dimpro.calculated.energyAssemblies.schema, zoneSchema:dimpro.calculated.energyZones.schema, openingSchema:dimpro.calculated.energyOpenings.schema, demandSchema:dimpro.calculated.energyDemand.schema, initialZoneState, connectionState, zoneTraceState, storedZones, energyOpeningId, detailedOpeningState, openingTraceState, storedOpenings, structuralFixture, demandWithoutSystems, energySystemId, demandResultState, demandTraceState, storedDemand, demandTotals:dimpro.calculated.energyDemand.totals, winWattFile, winWattSchema:winWatt.schema, openingWinWattTotals:winWatt.openingThermalTotals, demandWinWattTotals:winWatt.demandTotals, uState, solverState, pdfFile, migrated, responsive, consoleErrors:relevantConsoleErrors, pageErrors }, null, 2));
