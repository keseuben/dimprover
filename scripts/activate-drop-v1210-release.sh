...
EXPECTED_PREVIOUS=".next-v129-release-final"
EXPECTED_BUILD="YtYjsCjg5WLQFundZIY8j"
STAMP="$(date +%Y%m%d_%H%M%S)"
BACKUP="$ROOT/backups/drop_v1210_release_activation_$STAMP"
...
cd "$ROOT"
[[ -f "$RELEASE/BUILD_ID" && -f "$RELEASE/standalone/server.js" ]] || { echo "Hiányos DROP 1.2.10 release." >&2; exit 2; }
[[ "$(cat "$RELEASE/BUILD_ID")" == "$EXPECTED_BUILD" ]] || { echo "Váratlan DROP 1.2.10 BUILD_ID." >&2; exit 3; }
CURRENT="$(tr -d '\r\n' < .dimprover/active-next-release)"
if [[ "$CURRENT" == "$RELEASE" ]]; then echo "DROP 1.2.10 már aktív."; exit 0; fi
...
chmod 600 "$BACKUP"/*
printf '%s\n' "${BACKUP#$ROOT/}" > .work_drop_v1210_release_activation_backup
chmod 600 .work_drop_v1210_release_activation_backup
rollback(){
  local code="${1:-1}"
  trap - ERR
  echo "[DROP 1.2.10] Aktiválási hiba, automatikus rollback." >&2
...
for _ in $(seq 1 90); do
  if curl -fsS --max-time 8 -H 'Host: drop.dimpro.hu' http://127.0.0.1:3000/api/drop/health > /tmp/drop-v1210-live-health.json 2>/dev/null \
    && curl -fsS --max-time 8 -H 'Host: drop.dimpro.hu' http://127.0.0.1:3000/api/dimpro-identity/health > /tmp/drop-v1210-live-identity.json 2>/dev/null; then READY=1; break; fi
  sleep 2
done
[[ "$READY" == "1" ]]
node - <<'NODE'
const fs=require('fs');const h=JSON.parse(fs.readFileSync('/tmp/drop-v1210-live-health.json','utf8'));const i=JSON.parse(fs.readFileSync('/tmp/drop-v1210-live-identity.json','utf8'));
if(h.version!=='DROP 1.2.10') throw new Error(`Várt DROP 1.2.10, kapott ${h.version}`);
...
NODE
curl -fsS --max-time 20 https://drop.dimpro.hu/api/drop/health > /tmp/drop-v1210-live-https-health.json
curl -sSI --max-time 20 https://drop.dimpro.hu/send | grep -i '^permissions-policy:' | grep -q 'microphone=(self)'
curl -fsS --max-time 20 https://drop.dimpro.hu/drop-sw.js > /tmp/drop-v1210-live-sw.js
grep -q 'DROP_SW_VERSION = "DROP 1.2.10"' /tmp/drop-v1210-live-sw.js
grep -q 'dimpro-drop-static-v1210' /tmp/drop-v1210-live-sw.js
node - <<'NODE'
const fs=require('fs');const h=JSON.parse(fs.readFileSync('/tmp/drop-v1210-live-https-health.json','utf8'));if(h.version!=='DROP 1.2.10'||h.coreReady!==true||h.readiness?.dimproSend!==true||h.readiness?.pwaUpdateNotification!==true||h.worker?.claimLimit!==4) throw new Error('HTTPS DROP 1.2.10 health hibás.');
NODE
pm2 save >/dev/null
...
trap - ERR
echo "DROP 1.2.10 aktiválva. Release: $RELEASE; build: $EXPECTED_BUILD; rollback: $EXPECTED_PREVIOUS"
