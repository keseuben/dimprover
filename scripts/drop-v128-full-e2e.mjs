...
const PORT = Number(process.env.DROP_V128_E2E_PORT || 3120);
const FORWARDED_IP = '198.51.100.211';
const USER_AGENT = 'DIMPRO DROP 1.2.8 full UX Identity E2E';
...
async function main() {
  ...
  const evidence = { ok: false, version: 'DROP 1.2.8', checks: [], auditEvents: [], rateLimit: null, accountingCount: null, packagePublicCode: null, projectCode: null, cleanup: { s3Objects: 0 } };
  ...
  try {
    ...
    const movedGroupCreate = await api('/api/drop/access/groups', { method: 'POST', bearer: uploadToken, body: { name: 'E2E áthelyezett csoport' } });
    ...
    const zipResponse = await api('/api/drop/downloads/package/zip', {
      method: 'POST', host: 'drop.dimpro.hu', body: { token: rawDownloadToken, brandPrefix: true, requestId: `v128_${unique}`, includePdf: true, includeTxt: true, pdfImagesPerPage: 4 },
    });
    ...
    const out = '.work_drop_v128_full_e2e_result.json';
...
main().catch((error) => {
  console.error(`DROP_V128_FULL_E2E_FAILED: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});
