...
const heartbeatType=bridge.slice(bridge.indexOf("export type WindowsBridgeHeartbeat"),bridge.indexOf("export function getWindowsBridgeReadiness"));
check("Heartbeat nem tartalmaz secretet",heartbeatType.includes("export type WindowsBridgeHeartbeat")&&!/password|secret|tokenValue|apiKey/i.test(heartbeatType));
...
check("P8 foundation/P8.1 pairing réteg továbbra sem hoz command API-t",!fs.existsSync(path.join(root,"app/api/dev/terminal-hub/windows-bridge/command")));
check("P8 pairing API-k execution nélkül külön rétegben maradnak",fs.existsSync(path.join(root,"app/api/dev/terminal-hub/windows-bridge/pairings/route.ts"))&&!fs.existsSync(path.join(root,"app/api/dev/terminal-hub/windows-bridge/execute")));
...
check("P8 UI nincs Connect/Run PowerShell/Futtatás gomb",!/>\s*(?:Connect|Run PowerShell|Futtatás)\s*</i.test(ui));
check("P8/P8.1 UI explicit execution OFF",ui.includes("PowerShell execution")&&ui.includes("P8.1 heartbeat parancslistája kötelezően üres"));
