...
const PORT=Number(process.env.DROP_V127_BROWSER_PORT||3120);
...
const fullName=`DROP 1.2.7 Browser User ${stamp}`;
const email=`drop-v127-browser-${stamp}@example.invalid`;
const phone='+36 30 987 6543';
const organizationName=`DIMPRO V127 Browser Org ${stamp}`;
...
try{
 ...
 const r=await page.goto(`${LICENSE}/drive/drop/public-workflows?v127=1`,{waitUntil:'networkidle2',timeout:120000});
 pass('admin-http',r?.status()===200,String(r?.status()));
 await page.waitForFunction(()=>(document.body.textContent||'').includes('DROP 1.2.7')&&(document.body.textContent||'').includes('Új Send entitlement'),{timeout:30000});
 pass('v127-admin-visible',true);
 ...
 const sr=await sendPage.goto(`${DROP}/send?v127-login=1`,{waitUntil:'networkidle2',timeout:120000});
 ...
 await sendPage.type('input[placeholder="Név"]','Csató Ferenc V127',{delay:15});
 await sendPage.type('input[placeholder="E-mail-cím"]','csato.ferenc.v127.browser@example.invalid',{delay:15});
 ...
 await sendPage.waitForFunction(()=>{const t=document.body.textContent||'';return t.includes('csato.ferenc.v127.browser@example.invalid')&&t.includes('A címjegyzék mentve.');},{timeout:20000});
 const contactRow=await client.from('dimpro_send_recipients').select('id,recipient_name,recipient_email,organization_name,active').eq('entitlement_id',fixture.entitlementId).eq('recipient_email','csato.ferenc.v127.browser@example.invalid').single(); if(contactRow.error)throw contactRow.error;
