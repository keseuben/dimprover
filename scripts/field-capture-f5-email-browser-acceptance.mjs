...
try {
    ...
    if (url.includes(`/api/field-capture/sessions/${SERVER_SESSION_ID}/report-email`)) {
      ...
      if (request.method() === "GET") {
        void request.respond({ status: 200, contentType: "application/json", body: JSON.stringify({
          ...
          status: { configured: true, from: "ertesites.drop@dimpro.hu", profileId: "drop", recipientMode: "free_entry", maxRecipients: 3, suggestedRecipients: ["terep-ui-test@example.com"] },
        }) });
        return;
      }
      if (request.method() === "POST") {
        void request.respond({ status: 200, contentType: "application/json", body: JSON.stringify({
          ok: true,
          result: { messageId: "f5-browser-test-message", recipients: ["terep-ui-test@example.com"], attachmentName: "DIMPRO_Terepi_osszesito_F5.pdf", subject: "F5 browser riport" },
        }) });
        return;
      }
    }
  ...
  await authenticate();
  pass("F5 kliensverzió 0.4.3-dev", await page.evaluate(() => (document.body.textContent || "").includes("V0.4.3-dev")));
