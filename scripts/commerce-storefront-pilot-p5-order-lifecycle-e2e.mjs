...
const idempotencyKey = `p5-persistent-checkout-${marker}`;
...
try {
  ...
  pass("Commerce schema is P5 0.1.16 / 17", meta.data.schema_version === "0.1.16" && Number(meta.data.migration_count) === 17, JSON.stringify(meta.data));
  ...
  pass("P5 E2E starts without foreign due mirror jobs", Number(due.count || 0) === 0, String(due.count || 0));
  ...
  const checkout = await api("/api/aruter/public-checkouts", {
    ...
    body: {
      ...
      customerName: "DIMARO P5 Persistent E2E",
      phone: "+36 30 000 0000",
      email: "p5-persistent-e2e@example.invalid",
      note: `P5 persistent lifecycle E2E ${marker}`,
  ...
  if (legacyOrderId) await admin.from("commerce_storefront_orders").update({ deleted_at: now }).eq("organization_id", ORGANIZATION_ID).eq("legacy_order_id", legacyOrderId).is("deleted_at", null);
}
