...
const PORT = Number(process.env.DROP_V129_E2E_PORT || 3120);
const FORWARDED_IP = '198.51.100.211';
const USER_AGENT = 'DIMPRO DROP 1.2.9 full UX Identity E2E';
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = http.request({
      ...
      res.on('end', () => {
        ...
        resolve({ status: res.statusCode || 0, raw, buffer: Buffer.concat(chunks), json, headers: res.headers, setCookie: Array.isArray(setCookie) ? setCookie : setCookie ? [setCookie] : [] });
      });
    });
    req.on('error', reject);
...
async function main() {
  ...
  const evidence = { ok: false, version: 'DROP 1.2.9', checks: [], auditEvents: [], rateLimit: null, accountingCount: null, packagePublicCode: null, projectCode: null, cleanup: { s3Objects: 0 } };
  ...
  try {
    ...
    let uploadToken = packageCreate.json?.created?.uploadToken;
    ...
    const resumeBeforeUpload = await api('/api/drop/public/packages/resume?workflowType=send', { cookie: dropCookie });
    assert.equal(resumeBeforeUpload.status, 200, `resume before upload: ${resumeBeforeUpload.status} ${resumeBeforeUpload.raw}`);
    check('resume-same-package-before-upload', resumeBeforeUpload.json?.resume?.package?.id === fixture.packageId, JSON.stringify(resumeBeforeUpload.json?.resume?.package || null));
    check('resume-entitlement-binding-returned', resumeBeforeUpload.json?.resume?.workflow?.dimproSendEntitlementId === fixture.entitlementId, JSON.stringify(resumeBeforeUpload.json?.resume?.workflow || null));
    check('resume-rotated-upload-capability', typeof resumeBeforeUpload.json?.resume?.uploadToken === 'string' && resumeBeforeUpload.json.resume.uploadToken.length > 30);
    uploadToken = resumeBeforeUpload.json.resume.uploadToken;
    ...
    const group = await api('/api/drop/access/groups', { method: 'POST', bearer: uploadToken, body: { name: 'Bocskai', description: 'DROP 1.2.9 PDF grid E2E csoport.' } });
    ...
    const movedDisplayName = '260808_2200_260808_F0001_athelyezett.png';
    ...
    const movedGroupCreate = await api('/api/drop/access/groups', { method: 'POST', bearer: uploadToken, body: { name: 'Kossuth' } });
    assert.ok([200,201].includes(movedGroupCreate.status), `move group create: ${movedGroupCreate.status} ${movedGroupCreate.raw}`);
    const movedGroupId=movedGroupCreate.json?.group?.id;
    const moveFile=await api('/api/drop/access/files/group',{method:'PATCH',bearer:uploadToken,body:{fileId:fixture.fileId,groupId:movedGroupId,displayName:movedDisplayName}});
    assert.equal(moveFile.status,200,`file group move: ${moveFile.status} ${moveFile.raw}`);
    const movedRow=await client.from('drop_files').select('group_id,display_name').eq('id',fixture.fileId).single(); if(movedRow.error)throw movedRow.error;
    check('uploaded-file-group-moved-server-side',movedRow.data.group_id===movedGroupId,JSON.stringify(movedRow.data));
    check('uploaded-file-display-name-updated',movedRow.data.display_name===movedDisplayName,JSON.stringify(movedRow.data));

    // DROP 1.2.9 valós PDF-rács fixture: Bocskai 4 kép, Kossuth 7 kép.
    // Az első, már tiszta kép Kossuth csoportban van; ehhez adunk 4 Bocskai + 6 Kossuth képet.
    const gridIntent = await api('/api/drop/access/uploads/intent', { method: 'POST', bearer: uploadToken, body: { count: 10 } });
    assert.equal(gridIntent.status, 201, `grid upload intent: ${gridIntent.status} ${gridIntent.raw}`);
    const gridIntents = gridIntent.json?.intents || [];
    check('grid-upload-intents-ten', gridIntents.length === 10, String(gridIntents.length));
    await sleep(1700);
    const gridFileIds = [];
    for (let index = 0; index < 10; index += 1) {
      const bocskai = index < 4;
      const targetGroupId = bocskai ? groupId : movedGroupId;
      const label = bocskai ? 'bocskai' : 'kossuth';
      const displayName = `260809_1000_260809_F${String(index + 2).padStart(4,'0')}_${label}_grid.png`;
      const gridInit = await api('/api/drop/access/uploads/init', { method: 'POST', bearer: uploadToken, body: {
        fileName: displayName,
        originalFileName: `GRID_${label}_${index + 1}.PNG`,
        displayFileName: displayName,
        sourceOriginalSizeBytes: bytes.length,
        sizeBytes: bytes.length,
        mimeType: 'image/png',
        groupId: targetGroupId,
        clientUploadId: `v129-grid-${unique}-${index}`,
        uploadedByName: 'DIMPRO Identity E2E felhasználó',
        uploadedByEmail: fixtureEmail,
        rulesAccepted: true,
        rulesVersion: 'DIMPRO-DROP-UPLOAD-HU-1.0',
        rulesAcceptedAt: new Date().toISOString(),
        robotGuard: { intentToken: gridIntents[index]?.token, website: '' },
      } });
      assert.equal(gridInit.status, 201, `grid upload init ${index}: ${gridInit.status} ${gridInit.raw}`);
      const gridInitialized = gridInit.json?.initialized;
      const gridFileId = String(gridInitialized?.file?.id || '');
      assert.ok(gridFileId.length > 20, `grid file id ${index}`);
      await uploadOne(gridInitialized, bytes);
      gridFileIds.push(gridFileId);
    }
    for (let round = 0; round < 4; round += 1) {
      const scan = await api('/api/drop/worker/run', { method: 'POST', host: '127.0.0.1', workerSecret: required('DROP_WORKER_SECRET'), body: { limit: 4, scanOnly: true } });
      assert.equal(scan.status, 200, `grid scan ${round}: ${scan.status} ${scan.raw}`);
      await sleep(250);
    }
    for (const id of gridFileIds) await waitClean(client, id);
    const gridRows = await client.from('drop_files').select('id,group_id,security_status').eq('package_id', fixture.packageId);
    if (gridRows.error) throw gridRows.error;
    const bocskaiCount=(gridRows.data||[]).filter(row=>row.group_id===groupId).length;
    const kossuthCount=(gridRows.data||[]).filter(row=>row.group_id===movedGroupId).length;
    check('grid-real-file-counts-4-and-7', bocskaiCount===4 && kossuthCount===7, `${bocskaiCount}/${kossuthCount}`);
    const longComment = `Hosszú megjegyzés: ${'helyszíni ellenőrzési megjegyzés '.repeat(35)}`;
    const gridComment = await api(`/api/drop/public/packages/${encodeURIComponent(fixture.packageId)}/comments`, { method: 'PUT', cookie: dropCookie, body: { fileId: gridFileIds[0], commentText: longComment } });
    assert.equal(gridComment.status, 200, `grid long comment: ${gridComment.status} ${gridComment.raw}`);
    const preferences = await api(`/api/drop/public/packages/${encodeURIComponent(fixture.packageId)}/preferences`, { method:'PATCH', cookie:dropCookie, body:{ appendGroupNameToFilename:true, exportGroupsAsFolders:true } });
    assert.equal(preferences.status, 200, `group preferences: ${preferences.status} ${preferences.raw}`);

    const firstComment = await api(`/api/drop/public/packages/${encodeURIComponent(fixture.packageId)}/comments`, { method: 'PUT', cookie: dropCookie, body: { fileId: fixture.fileId, commentText: 'Első karantén utáni megjegyzés.' } });
    assert.equal(firstComment.status, 200, `comment put 1: ${firstComment.status} ${firstComment.raw}`);
    const secondComment = await api(`/api/drop/public/packages/${encodeURIComponent(fixture.packageId)}/comments`, { method: 'PUT', cookie: dropCookie, body: { fileId: fixture.fileId, commentText: 'Módosított megjegyzés vírusellenőrzés után.' } });
    assert.equal(secondComment.status, 200, `comment put 2: ${secondComment.status} ${secondComment.raw}`);
    const commentRows = await client.from('drop_comments').select('id,comment_text,status').eq('package_id', fixture.packageId).eq('file_id', fixture.fileId).eq('author_user_id', 'drop-public-sender').eq('status', 'active');
    if (commentRows.error) throw commentRows.error;
    check('quarantine-comment-upsert-single-active', (commentRows.data || []).length === 1, JSON.stringify(commentRows.data));
    check('quarantine-comment-latest-text', commentRows.data?.[0]?.comment_text === 'Módosított megjegyzés vírusellenőrzés után.', JSON.stringify(commentRows.data));
    ...
    check('notification-email-sent', workflowAfter.data.notification_status === 'sent', String(workflowAfter.data.notification_status));
    ...
    try {
      ...
      await page.waitForFunction((name) => (document.body.textContent || '').includes('Képalbum és fájlok') && (document.body.textContent || '').includes(name), { timeout: 60000 }, movedDisplayName);
      await page.evaluate((name) => { const img=[...document.querySelectorAll('img')].find((item)=>item.alt===name); img?.scrollIntoView({block:'center'}); }, movedDisplayName);
      const album = await page.evaluate((name) => {
        ...
        return {
          ...
          groupSummary: body.includes('Csoportok összesítője'),
          groupedContent: body.includes('Csoportosított csomagtartalom'),
          groupOpenByDefault: Boolean([...document.querySelectorAll('button[aria-expanded]')].find(button=>button.getAttribute('aria-expanded')==='true')),
      ...
      check('album-thumbnail-visible', album.image && album.filename, JSON.stringify(album));
      const previewHttp = album.previewUrl ? await fetch(album.previewUrl) : null;
      check('album-thumbnail-signed-url-http', Boolean(previewHttp && previewHttp.ok && String(previewHttp.headers.get('content-type') || '').startsWith('image/')), previewHttp ? `${previewHttp.status} ${previewHttp.headers.get('content-type')}` : 'no preview url');
      ...
      check('download-group-summary-visible', album.groupSummary && album.groupedContent, JSON.stringify(album));
      check('download-groups-open-by-default', album.groupOpenByDefault, JSON.stringify(album));
      const collapseState=await page.evaluate(()=>{const button=[...document.querySelectorAll('button[aria-expanded]')].find(item=>item.getAttribute('aria-expanded')==='true');if(!button)return null;button.click();return button.getAttribute('aria-expanded');});
      await sleep(100);
      check('download-group-can-collapse',collapseState==='true' && await page.evaluate(()=>[...document.querySelectorAll('button[aria-expanded]')].some(item=>item.getAttribute('aria-expanded')==='false')));
      const reopened=await page.evaluate(()=>{const summary=[...document.querySelectorAll('button')].find(button=>(button.textContent||'').includes('Kossuth')&&!button.hasAttribute('aria-expanded'));summary?.click();return Boolean(summary);});
      await sleep(120);
      check('download-summary-reopens-group',reopened && await page.evaluate(()=>[...document.querySelectorAll('button[aria-expanded]')].some(item=>item.getAttribute('aria-expanded')==='true')));
    ...
    const pdfResponse = await api('/api/drop/downloads/package/report', {
      method: 'POST', host: 'drop.dimpro.hu', body: { token: rawDownloadToken, imagesPerPage: 4 },
    });
    check('direct-pdf-http', pdfResponse.status === 200, `${pdfResponse.status} ${pdfResponse.raw.slice(0,160)}`);
    check('direct-pdf-content-type', String(pdfResponse.headers?.['content-type'] || '').includes('application/pdf'));
    check('direct-pdf-payload', pdfResponse.buffer.subarray(0,4).toString('ascii') === '%PDF' && pdfResponse.buffer.length > 1000, String(pdfResponse.buffer.length));
    await writeFile('.work_drop_v129_grid4.pdf', pdfResponse.buffer);
    const pdf6Response = await api('/api/drop/downloads/package/report', { method:'POST', host:'drop.dimpro.hu', body:{ token:rawDownloadToken, imagesPerPage:6 } });
    check('direct-pdf6-http', pdf6Response.status===200, `${pdf6Response.status}`);
    check('direct-pdf6-payload', pdf6Response.buffer.subarray(0,4).toString('ascii')==='%PDF' && pdf6Response.buffer.length>1000, String(pdf6Response.buffer.length));
    await writeFile('.work_drop_v129_grid6.pdf', pdf6Response.buffer);

    const textResponse = await api('/api/drop/downloads/package/text', {
      method: 'POST', host: 'drop.dimpro.hu', body: { token: rawDownloadToken },
    });
    check('direct-txt-http', textResponse.status === 200, `${textResponse.status} ${textResponse.raw.slice(0,160)}`);
    check('direct-txt-content-type', String(textResponse.headers?.['content-type'] || '').includes('text/plain'));
    check('direct-txt-filename-content', textResponse.raw.includes(movedDisplayName), textResponse.raw.slice(0,400));
    check('direct-txt-bocskai-group', textResponse.raw.includes('=== BOCSKAI · 4 KÉP/FÁJL ==='));
    check('direct-txt-kossuth-group', textResponse.raw.includes('=== KOSSUTH · 7 KÉP/FÁJL ==='));
    check('direct-txt-group-order', textResponse.raw.indexOf('=== BOCSKAI') < textResponse.raw.indexOf('=== KOSSUTH'));
    check('direct-txt-full-long-comment', textResponse.raw.includes('helyszíni ellenőrzési megjegyzés'));
    await writeFile('.work_drop_v129_grouped.txt', textResponse.buffer);

    const zipResponse = await api('/api/drop/downloads/package/zip', {
      method: 'POST', host: 'drop.dimpro.hu', body: { token: rawDownloadToken, brandPrefix: true, requestId: `v129_${unique}`, includePdf: true, includeTxt: true, pdfImagesPerPage: 4 },
    });
    ...
    console.log('ZIP_DIAGNOSTICS', JSON.stringify({status:zipResponse.status,contentType:zipContentType,disposition,size:zipResponse.buffer.length}));
    ...
    const zip = await JSZip.loadAsync(zipResponse.buffer);
    const zipNames = Object.keys(zip.files);
    check('zip-contains-source-file', zipNames.some((name) => name.endsWith(movedDisplayName)), zipNames.join(','));
    check('zip-contains-bocskai-folder', zipNames.some((name) => name.includes('Bocskai/') && name.endsWith('.png')), zipNames.join(','));
    check('zip-contains-kossuth-folder', zipNames.some((name) => name.includes('Kossuth/') && name.endsWith('.png')), zipNames.join(','));
    check('zip-contains-pdf', zipNames.some((name) => name.endsWith('_csomagriport_4kep_oldal.pdf')), zipNames.join(','));
    check('zip-contains-txt', zipNames.some((name) => name.endsWith('_megjegyzesek.txt')), zipNames.join(','));
    ...
    const out = '.work_drop_v129_full_e2e_result.json';
...
main().catch((error) => {
  console.error(`DROP_V129_FULL_E2E_FAILED: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});
