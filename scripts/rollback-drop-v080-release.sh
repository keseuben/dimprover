...
set -Eeuo pipefail
ROOT="/root/dimprover"
BACKUP_REF="$ROOT/.work_drop_v080_release_activation_backup"
if [[ ! -f "$BACKUP_REF" ]]; then echo "Hiányzó DROP 0.8.0 rollback-hivatkozás: $BACKUP_REF" >&2; exit 2; fi
...
if [[ ! -f "$ENV_BACKUP" || ! -f "$POINTER_BACKUP" ]]; then echo "A DROP 0.8.0 rollback-mentés hiányos: $BACKUP_DIR" >&2; exit 4; fi
...
cp -a "$ROOT/.env.local" "$ROOT/.env.local.before-v080-rollback-$(date +%Y%m%d_%H%M%S)"
...
echo "DROP 0.8.0 rollback kész. Aktív release: $PREVIOUS_RELEASE; build: $ACTIVE_BUILD"
