...
function actions(page){return page.$eval(`[data-task-id="${taskId}"]`,node=>({status:node.getAttribute("data-status"),buttons:[...node.querySelectorAll("button")].map(b=>b.textContent?.trim()||""),text:node.textContent||"",overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth}))}
try{
