...
const USER_AGENT = 'DIMPRO DROP 1.2.2 full UX Identity E2E';
...
async function main() {
  ...
  const evidence = { ok: false, version: 'DROP 1.2.2', checks: [], auditEvents: [], rateLimit: null, accountingCount: null, packagePublicCode: null, projectCode: null, cleanup: { s3Objects: 0 } };
  ...
  try {
    ...
    const zipResponse = await api('/api/drop/downloads/package/zip', {
      method: 'POST', host: 'drop.dimpro.hu', body: { token: rawDownloadToken, brandPrefix: true, requestId: `v122_${unique}` },
    });
    ...
    const out = '.work_drop_v122_full_e2e_result.json';
...
main().catch((error) => {
  console.error(`DROP_V120_FULL_E2E_FAILED: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});
