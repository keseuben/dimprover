...
const downloadDir = process.env.DIMPRO_TEST_DOWNLOAD_DIR || '/tmp/dimpro_energy_v073_downloads';
...
async function createProjectAndSurvey(page) {
  ...
  await page.type('input[placeholder="Projekt neve *"]', 'V073 energetikai teszt');
  await page.type('input[placeholder="Projektkód"]', 'V073');
  ...
  await waitText(page, 'V073 energetikai teszt');
  ...
  await inputs[0].type('V073 energetikai mintafelmérés');
...
(async () => {
  ...
  pass('Hatfüles energetikai munkatér zóna- és U-érték nézettel megnyílik');
  ...
  pass('Állapotlap geometria-, U-érték- és zónamotor verziót mutat');
  ...
  assert(dimpro.schema === 'dimpro.property-survey.v0.7.3', `Hibás .dimpro séma: ${dimpro.schema}`);
  ...
  pass('.dimpro v0.7.3 export tartalmazza a geometriát, zónákat, U-értéket és anyagteret');
  ...
  assert(winWatt.schema === 'dimpro.winwatt-compatible.v0.7.3', `Hibás WinWatt séma: ${winWatt.schema}`);
  ...
  pass('WinWatt-előkészítő zónákat, fűtetlen teret és kapcsolatokat tartalmaz');
  ...
  pass('Vektoros PDF külön zóna-, fűtetlen tér- és U-érték összesítő oldalt tartalmaz');
  ...
  await page.evaluate(() => {
    ...
    for (const survey of workspace.surveys) { delete survey.draft.energyProjectSettings; delete survey.draft.energyZoneWorkspace; delete survey.draft.materialWorkspace; }
  ...
  const migrated = await page.evaluate(() => {
    const workspace = JSON.parse(localStorage.getItem('dimpro-property-survey-workspace-v1'));
    const survey = workspace.surveys.find((item) => item.id === workspace.activeSurveyId);
    return { energySchema: survey.draft.energyProjectSettings?.schemaVersion, zoneSchema: survey.draft.energyZoneWorkspace?.schemaVersion, zoneCount: survey.draft.energyZoneWorkspace?.zones?.length, assignedRooms: Object.keys(survey.draft.energyZoneWorkspace?.roomAssignments || {}).length, materialSchema: survey.draft.materialWorkspace?.schemaVersion, materialProjectId: survey.draft.materialWorkspace?.projectCatalog?.projectId, projectId: survey.projectId };
  });
  assert(migrated.energySchema === 1 && migrated.zoneSchema === 1 && migrated.zoneCount === 1 && migrated.assignedRooms === 6 && migrated.materialSchema === 1 && migrated.materialProjectId === migrated.projectId, `Régi projekt migrációja hibás: ${JSON.stringify(migrated)}`);
  pass('v0.6.x projekt energetikai, zóna- és anyagtér-migrációja működik');
