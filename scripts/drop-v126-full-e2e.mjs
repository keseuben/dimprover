...
const PORT = Number(process.env.DROP_V126_E2E_PORT || 3120);
const FORWARDED_IP = '198.51.100.211';
const USER_AGENT = 'DIMPRO DROP 1.2.6 full UX Identity E2E';
...
async function main() {
  ...
  const evidence = { ok: false, version: 'DROP 1.2.6', checks: [], auditEvents: [], rateLimit: null, accountingCount: null, packagePublicCode: null, projectCode: null, cleanup: { s3Objects: 0 } };
  ...
  try {
    ...
    const zipResponse = await api('/api/drop/downloads/package/zip', {
      method: 'POST', host: 'drop.dimpro.hu', body: { token: rawDownloadToken, brandPrefix: true, requestId: `v126_${unique}`, includePdf: true, includeTxt: true, pdfImagesPerPage: 4 },
    });
