...
check(wrapper.includes('BENJADMIN_DEV_CANONICAL_ROOT') && wrapper.includes('/srv/dimpro-dev/worktrees/'), "canonical DEV worktree explicitly overridable but DEV-root constrained");
check(wrapper.includes('BENJADMIN_DEV_CANONICAL_BRANCH') && wrapper.includes('git check-ref-format --branch'), "canonical branch explicitly overridable and validated");
check(wrapper.includes('BENJADMIN_DEV_CANONICAL_GIT') && wrapper.includes('/srv/dimpro-dev/repositories/dimprover.git'), "canonical bare repository remains fixed to DEV Git");
