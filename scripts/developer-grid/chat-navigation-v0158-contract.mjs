...
check("ChatGPT window.open is denied as separate window",()=>{const handler=main.indexOf("view.webContents.setWindowOpenHandler");const a=main.indexOf("if (isChatGptUrl(url))",handler);const b=main.indexOf("setImmediate(() => void shell.openExternal",a);const block=main.slice(a,b);assert.ok(a>handler&&b>a);assert.match(block,/view\.webContents\.loadURL\(url\)/);assert.match(block,/return \{ action: "deny" \}/);assert.doesNotMatch(block,/action: "allow"/);});
