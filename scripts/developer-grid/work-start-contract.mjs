...
check(ui.includes("if(state.workStartBusy||state.taskBridgeBusy)return") && ui.includes("state.workStartBusy||state.workResumeBusy||state.taskBridgeBusy||!valid"), "double-submit guarded client-side for ChatGPT and Codex");
...
check(engine.includes('.filter((task) => !["queued", "ready"].includes') && engine.includes("handoffWorkerCode || contextSnapshot?.workerCode || previousWorkerCode"), "continuity ignores unstarted queue tasks and prefers worked handoff evidence");
