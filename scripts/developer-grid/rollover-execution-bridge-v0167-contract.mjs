...
check("execution request proof rejects generic local override; V0168 recovery proof is exact-bound only",()=>{
  ...
  const body=main.slice(a,b);
  assert.ok(body.includes('executionAuthorityRecoveryVersion || "").toUpperCase() === "V0168"'));
  assert.ok(body.includes("executionAuthorityTaskId"));
  assert.ok(body.includes("executionAuthoritySessionId"));
  assert.ok(body.includes("executionAuthoritySourceHead"));
  assert.ok(body.includes("const expectedProof = recoveredProofBound ? recoveredProof : resolvedExecutionProofSha256(task);"));
