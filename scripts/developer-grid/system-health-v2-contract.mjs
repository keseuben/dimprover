...
check("Supabase traffic uses read-only Management API instead of database polling", () => { assert.match(facade, /SUPABASE_MANAGEMENT_API/); assert.match(facade, /usage\.api-counts/); assert.match(facade, /BENJADMIN_SUPABASE_ANALYTICS_TOKEN/); assert.doesNotMatch(facade, /@supabase|createClient\(|\.from\(/i); });
check("Supabase analytics token supports secure server-side token file without service-role fallback", () => { assert.match(facade, /DEFAULT_SUPABASE_ANALYTICS_TOKEN_FILE/); assert.match(facade, /analytics-usage-read\.token/); assert.match(facade, /supabaseAnalyticsToken/); assert.doesNotMatch(facade, /SUPABASE_SERVICE_ROLE_KEY/); });
