...
check("Supabase project mapping is admin-only and least privilege", () => { assert.match(supabaseRoute, /configureSupabaseMonitoringProject/); assert.match(supabaseRoute, /export async function PUT/); assert.match(supabaseConfig, /getSupabaseMonitoringProjects/); assert.match(supabaseConfig, /SUPABASE_PROJECT_REF_INVALID/); assert.match(supabaseConfig, /validateSupabaseAnalyticsToken\(token, \[projectRef\]\)/); assert.match(supabaseSetup, /PROD projekt külön bekötése/); });
check("Supabase project metrics remain isolated and billing egress is not inferred", () => { assert.match(facade, /Promise\.all\(projects\.map/); assert.match(facade, /supabase-traffic-prod/); assert.match(facade, /egressBytes: null/); assert.doesNotMatch(facade, /trafficCache\.value = \[await inspectSupabaseTraffic/); });
