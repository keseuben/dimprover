...
check("current package keeps v0.1.37 recovery", () => assert.equal(pkg.version, "0.1.65"));
check("current backend keeps v0.1.37 recovery", () => assert.match(types, /DEVELOPER_GRID_VERSION = "0.1.65-dev"/));
...
check("shared BOOT ACK processor persists authoritative ACK", () => {
  ...
  assert.match(block, /ackRecoverySource: hasBootAck \? source : `\$\{source\}:STAGE1_PASS_FALLBACK`/);
});
