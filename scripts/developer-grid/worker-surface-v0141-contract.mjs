...
check("selector exposes ChatGPT and Codex while Work stays planned", () => { assert.match(html,/<option value="CHATGPT">ChatGPT<\/option>/); assert.match(html,/<option value="CODEX">Codex<\/option>/); assert.match(html,/<option value="WORK" disabled>Work · v0\.1\.57<\/option>/); });
...
check("Work adapter remains fail-closed for v0.1.56", () => assert.match(adapter,/WORK_SURFACE_PLANNED_V0156/));
...
check("Codex uses Task Bridge while Work remains fail-closed", () => { assert.match(context,/startDeveloperGridTaskBridge/); assert.match(context,/OPENAI FIRST-PARTY · TASK BRIDGE/); assert.match(workStart,/CODEX_TASK_BRIDGE_REQUIRED/); assert.match(workStart,/WORK_SURFACE_PLANNED_V0156/); });
...
console.log(`Developer Grid Worker Surface Selector v0.1.56 contract PASS · ${n}/${n}`);
