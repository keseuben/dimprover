...
check("desktop version v0.1.51", () => assert.equal(pkg.version, "0.1.51"));
check("backend version v0.1.51-dev", () => assert.match(types, /DEVELOPER_GRID_VERSION = "0\.1\.51-dev"/));
...
check("selector exposes ChatGPT and Codex while Work stays planned", () => { assert.match(html,/<option value="CHATGPT">ChatGPT<\/option>/); assert.match(html,/<option value="CODEX">Codex<\/option>/); assert.match(html,/<option value="WORK" disabled>Work · v0\.1\.52<\/option>/); });
...
check("Work adapter remains fail-closed for v0.1.52", () => assert.match(adapter,/WORK_SURFACE_PLANNED_V0152/));
...
check("Codex uses Task Bridge while Work remains fail-closed", () => { assert.match(context,/startDeveloperGridTaskBridge/); assert.match(context,/OPENAI FIRST-PARTY · TASK BRIDGE/); assert.match(workStart,/CODEX_TASK_BRIDGE_REQUIRED/); assert.match(workStart,/WORK_SURFACE_PLANNED_V0152/); });
...
console.log(`Developer Grid Worker Surface Selector v0.1.51 contract PASS · ${n}/${n}`);
