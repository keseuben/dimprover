...
check(source.includes("BUILD_CPUS=2") && source.includes("CPU_QUOTA_PERCENT=200") && source.includes('NEXT_BUILD_CPUS="$BUILD_CPUS"'), "adaptive two-CPU build profile");
check(source.includes("5 * 1024 * 1024") && source.includes("nproc"), "two-CPU profile requires five GiB available memory and host capacity");
...
check(source.includes("MemoryHigh=4300M") && source.includes("MemoryMax=5000M"), "memory pressure ceiling tuned for canonical DEV");
check(source.includes("RuntimeMaxSec=2700s"), "stuck canonical build has hard 45 minute runtime ceiling");
