...
set -Eeuo pipefail
ROOT="/root/dimprover"
CURRENT=".next-v1210-release-final"
...
for _ in $(seq 1 90); do
  if curl -fsS --max-time 8 -H 'Host: drop.dimpro.hu' http://127.0.0.1:3000/api/drop/health >/tmp/drop-v1210-rollback-health.json 2>/dev/null; then break; fi
  sleep 2
done
node - <<'NODE'
const h=require('/tmp/drop-v1210-rollback-health.json');if(h.version!=='DROP 1.2.9'||h.coreReady!==true)throw new Error(`Rollback health hibás: ${h.version}`);console.log(JSON.stringify({ok:true,version:h.version,coreReady:h.coreReady},null,2));
NODE
pm2 save >/dev/null
echo "DROP 1.2.10 rollback kész: $ROLLBACK"
