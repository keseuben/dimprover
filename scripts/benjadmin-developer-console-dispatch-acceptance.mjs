...
try {
  ...
  check("Task Ármin-AI ownership és routing", payload?.dispatch?.selectedWorkerId === "worker_arminai" && payload?.dispatch?.selectedWorkerCode === "ARMINAI" && payload?.task?.requestedWorkerId === "worker_arminai" && payload?.task?.status === "ready", JSON.stringify({ dispatch: payload?.dispatch || {}, task: payload?.task || {} }));
  check("Ben-AI fejlesztési időbecslés létrejött", Number(payload?.dispatch?.estimate?.minutes || 0) >= 30 && Number(payload?.dispatch?.estimate?.minMinutes || 0) >= 20 && Number(payload?.dispatch?.estimate?.maxMinutes || 0) >= Number(payload?.dispatch?.estimate?.minutes || 0), JSON.stringify(payload?.dispatch?.estimate || {}));
  ...
  check("DB task a Drive/Drop projekthez kötődik és READY ownership mentett", !taskRead.error && taskRead.data?.project_id === "project_drive_drop" && taskRead.data?.repository_id === "repo_dimprover" && taskRead.data?.requested_worker_id === "worker_arminai" && taskRead.data?.status === "ready" && taskRead.data?.metadata?.origin === "BENJADMIN_DEVELOPER_CONSOLE" && taskRead.data?.metadata?.ownerCode === "ARMINAI" && Number(taskRead.data?.metadata?.estimateMinutes || 0) >= 30, JSON.stringify(taskRead.data || {}));
