...
check("Hetzner Object Storage szolgáltatói modell valós mezőkkel jelenik meg", infrastructurePayload?.storages?.every((item) => item.provider === "HETZNER_OBJECT_STORAGE" && item.includedStorageBytes === 1000000000000 && item.bucketHardLimitBytes === 100000000000000), JSON.stringify(infrastructurePayload?.storages?.map((item) => ({ code: item.code, provider: item.provider, includedStorageBytes: item.includedStorageBytes, bucketHardLimitBytes: item.bucketHardLimitBytes }))));
check("Az 1 TB közös account-báziskeret nem kerül per-bucket hard keretként felhasználásra", infrastructurePayload?.storages?.every((item) => item.capacityBytes == null) && infrastructurePayload?.storageBilling?.scope === "ACCOUNT_SHARED" && infrastructurePayload?.storageBilling?.includedStorageBytes === 1000000000000, JSON.stringify({ storageBilling: infrastructurePayload?.storageBilling, capacities: infrastructurePayload?.storages?.map((item) => ({ code: item.code, capacityBytes: item.capacityBytes })) }));
...
check("AI strict átállási készenlét biztonságosan jelentett", entitlementResponse.status === 200 && typeof entitlementPayload?.entitlements?.summary?.aiRuntimeStrictReady === "boolean" && Array.isArray(entitlementPayload?.entitlements?.summary?.aiRuntimeStrictBlockers), JSON.stringify({ ready: entitlementPayload?.entitlements?.summary?.aiRuntimeStrictReady, blockers: entitlementPayload?.entitlements?.summary?.aiRuntimeStrictBlockers }));
...
try {
  ...
  check("S3 foglaltság, DIMPRO hard keret és Hetzner báziskeret látható", (desktop.leftTitle.match(/Tárhelyfoglaltság/g) || []).length === 2 && (desktop.leftTitle.match(/DIMPRO hard keret/g) || []).length === 2 && (desktop.leftTitle.match(/Hetzner báziskeret/g) || []).length === 2 && (desktop.leftTitle.match(/Foglalt:/g) || []).length === 2, desktop.leftTitle.slice(-1200));
  ...
  check("AI strict átállási állapot a csapatképernyőn látható", desktop.aiFinanceText.includes("strict:") && (desktop.aiFinanceText.includes("NEM KÉSZ") || desktop.aiFinanceText.includes("ELLENŐRZÉSRE KÉSZ")), desktop.aiFinanceText.slice(0, 700));
