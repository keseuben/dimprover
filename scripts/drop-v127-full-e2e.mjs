...
const PORT = Number(process.env.DROP_V127_E2E_PORT || 3120);
const FORWARDED_IP = '198.51.100.211';
const USER_AGENT = 'DIMPRO DROP 1.2.7 full UX Identity E2E';
...
async function main() {
  ...
  const evidence = { ok: false, version: 'DROP 1.2.7', checks: [], auditEvents: [], rateLimit: null, accountingCount: null, packagePublicCode: null, projectCode: null, cleanup: { s3Objects: 0 } };
  ...
  try {
    ...
    check('direct-txt-filename-content', textResponse.raw.includes(structuredFileName), textResponse.raw.slice(0,400));

    const zipResponse = await api('/api/drop/downloads/package/zip', {
      method: 'POST', host: 'drop.dimpro.hu', body: { token: rawDownloadToken, brandPrefix: true, requestId: `v127_${unique}`, includePdf: true, includeTxt: true, pdfImagesPerPage: 4 },
    });
    ...
    check('zip-contains-source-file', zipNames.some((name) => name.endsWith(structuredFileName)), zipNames.join(','));
    ...
    const out = '.work_drop_v127_full_e2e_result.json';
...
main().catch((error) => {
  console.error(`DROP_V127_FULL_E2E_FAILED: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});
