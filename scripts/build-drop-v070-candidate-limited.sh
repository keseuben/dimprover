...
set -Eeuo pipefail
cd /root/dimprover
TARGET=".next-v070-candidate"
LOG=".work_drop_v070_build.log"
EXIT_FILE=".work_drop_v070_build.exit"
cleanup_services() {
...
rm -rf "$TARGET"
set +e
NEXT_DIST_DIR="$TARGET" NEXT_SAFE_BUILD=1 NEXT_BUILD_CPUS=1 NODE_OPTIONS="--max-old-space-size=2200" ./node_modules/.bin/next build --webpack >"$LOG" 2>&1
CODE=$?
if [[ "$CODE" -eq 0 ]]; then
