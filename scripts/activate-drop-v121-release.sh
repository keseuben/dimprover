...
ROOT="/root/dimprover"
RELEASE=".next-v121-release-final"
EXPECTED_PREVIOUS=".next-v110-release-final"
EXPECTED_BUILD="tDQnHhBYliSgpELkCs5bv"
STAMP="$(date +%Y%m%d_%H%M%S)"
BACKUP="$ROOT/backups/drop_v121_release_activation_$STAMP"
TIMER_WAS_ACTIVE="$(systemctl is-active dimpro-drop-worker-v050.timer 2>/dev/null || true)"

cd "$ROOT"
[[ -f "$RELEASE/BUILD_ID" && -f "$RELEASE/standalone/server.js" ]] || { echo "Hiányos DROP 1.2.1 release." >&2; exit 2; }
[[ "$(cat "$RELEASE/BUILD_ID")" == "$EXPECTED_BUILD" ]] || { echo "Váratlan DROP 1.2.1 BUILD_ID." >&2; exit 3; }
CURRENT="$(tr -d '\r\n' < .dimprover/active-next-release)"
if [[ "$CURRENT" == "$RELEASE" ]]; then
  echo "DROP 1.2.1 már aktív."
  exit 0
fi
...
printf '%s\n' "${BACKUP#$ROOT/}" > .work_drop_v121_release_activation_backup
chmod 600 .work_drop_v121_release_activation_backup

rollback() {
  local code="${1:-1}"
  trap - ERR
  echo "[DROP 1.2.1] Aktiválási hiba, automatikus rollback indul." >&2
...
for _ in $(seq 1 90); do
  if curl -fsS --max-time 8 -H 'Host: drop.dimpro.hu' http://127.0.0.1:3000/api/drop/health > /tmp/drop-v121-live-health.json 2>/dev/null \
    && curl -fsS --max-time 8 -H 'Host: drop.dimpro.hu' http://127.0.0.1:3000/api/dimpro-identity/health > /tmp/drop-v121-live-identity.json 2>/dev/null; then
    READY=1
    break
  fi
  sleep 2
...
node - <<'NODE'
const fs=require('fs');
const h=JSON.parse(fs.readFileSync('/tmp/drop-v121-live-health.json','utf8'));
const i=JSON.parse(fs.readFileSync('/tmp/drop-v121-live-identity.json','utf8'));
if(h.version!=='DROP 1.2.1') throw new Error(`Várt DROP 1.2.1, kapott ${h.version}`);
...
curl -fsS --max-time 15 https://drop.dimpro.hu/api/drop/health > /tmp/drop-v121-live-https-health.json
node - <<'NODE'
const fs=require('fs');
const h=JSON.parse(fs.readFileSync('/tmp/drop-v121-live-https-health.json','utf8'));
if(h.version!=='DROP 1.2.1' || h.readiness?.identityCoreConsumer!==true || h.readiness?.dimproSend!==true) throw new Error('A HTTPS Drop health nem DROP 1.2.1 READY.');
NODE
...
echo "DROP 1.2.1 aktiválva. Release: $RELEASE; build: $EXPECTED_BUILD; rollback: $EXPECTED_PREVIOUS"
