...
check("device auth starts device and engine session heartbeat",()=>{assert.match(main,/credential\.mode === "device"\) \{ startDeviceHeartbeat\(\); startEngineSessionHeartbeat\(\); \}/);});
check("workspace lock stops both heartbeats with live client",()=>{assert.match(main,/function stopLiveClient\(\)[\s\S]*stopDeviceHeartbeat\(\)[\s\S]*stopEngineSessionHeartbeat\(\)/);assert.match(main,/function lockWorkspace[\s\S]*stopLiveClient\(\)/);});
