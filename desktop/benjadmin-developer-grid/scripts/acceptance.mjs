...
check("package version 0.1.66",()=>assert.equal(pkg.version,"0.1.66"));
...
check("control center shows Physical Windows E2E gate",()=>{assert.match(contextWorkspaceSource,/PHYSICAL WINDOWS E2E/);assert.match(contextWorkspaceSource,/getDeveloperGridWindowsE2E/);});
...
check("Task Launch V3 carries authoritative Central Core proof and BOOT ACK",()=>{assert.match(taskLaunchPrompt,/TASK_LAUNCH_V3/);assert.match(taskLaunchPrompt,/LAUNCH PACKET · AUTHORITATIVE/);assert.match(taskLaunchPrompt,/BOOT ACKNOWLEDGEMENT/);assert.match(taskLaunchPrompt,/CENTRAL CORE SOURCE PREFLIGHT PROOF/);assert.match(taskLaunchPrompt,/Source proof:/);assert.match(taskLaunchPrompt,/SOURCE_BASELINE_MISMATCH/);assert.match(taskLaunchPrompt,/EXECUTION_TOOL_UNAVAILABLE/);assert.match(taskLaunchPrompt,/Ne helyettesítsd \/root\/dimprover/);assert.match(taskLaunchPrompt,/MCP-mount hiánya önmagában NEM preflight blocker/);});
...
check("task inspector light status colors stay readable",()=>{assert.match(css,/:root\[data-theme="light"\] \.task-inspector-statebar span \{ color:#07536c;/);assert.match(css,/span\[data-state="PASS"\] \{ color:#07583f;/);assert.match(css,/span\[data-state="BLOCKED"\] \{ color:#8a3d00;/);assert.match(css,/:root\[data-theme="light"\] \.task-inspector-list article strong \{ color:#173448;/);assert.match(css,/\.task-inspector-list article\.is-pass strong \{ color:#07583f;/);assert.match(css,/\.task-inspector-list article\.is-blocked strong \{ color:#8a3d00;/);assert.match(css,/:root\[data-theme="light"\] \.task-inspector-dev-badge \{ color:#07536c;/);assert.match(css,/:root\[data-theme="light"\] \.task-inspector-prod-badge \{ color:#774a00;/);});
...
console.log(`BENJADMIN Developer Grid Desktop v0.1.66 DEV acceptance PASS · ${n}/${n}`);
