...
check("hover quick view is full width and stays above Central Core", () => { assert.match(css, /Developer Grid v0\.1\.13 · System Health overlay\/card refinement/); assert.match(css, /\.system-health-peek \{[\s\S]*left: 0;[\s\S]*right: 0;[\s\S]*width: auto/); assert.match(css, /body\.has-health-peek \.context-workspace[\s\S]*bottom: calc\(var\(--developer-footer-h\) \+ var\(--health-peek-h\)\)/); });
