...
check("live BOOT ACK monitor uses transcript-aware candidate capture",()=>{const start=main.indexOf("async function monitorWorkerBootAck(");const end=main.indexOf("\nasync function bindCurrentTaskConversation",start);const block=main.slice(start,end);assert.match(block,/captureLatestBootAckCandidate\(view\)/);assert.doesNotMatch(block,/captureLatestAssistantText\(view\)/)});
