...
const WORKER_SURFACE_LABELS = Object.freeze({ CHATGPT:"ChatGPT", CODEX:"Codex", WORK:"Work · v0.1.47" });
...
function renderConfig() {
  ...
  for (const cellConfig of state.config.cells) {
    ...
    if (surfaceSelect) {
      ...
      surfaceSelect.title = locked ? "Aktív task közben a worker surface nem váltható." : surfaceType === "CODEX" ? "Codex · OpenAI first-party Task Bridge" : surfaceType === "WORK" ? "Work · v0.1.47-ra előkészítve · visszaváltható" : "Worker felület kiválasztása";
    }
    ...
    if (emptyState) {
      ...
      if (surfaceType === "CODEX") {
        ...
        if (strong) strong.textContent = surfaceType === "WORK" ? "Work · OpenAI first-party surface · v0.1.47 adapter." : "A ChatGPT felület zárva.";
...
function renderSystemHealth() {
  ...
  if (columns) {
    if (health) {
      ...
      const trafficItems = Array.isArray(health.traffic) ? health.traffic : [];
      const trafficByEnvironment = (environment) => trafficItems.find((item) => item.environment === environment || item.id === (environment === "PROD" ? "supabase-traffic-prod" : "supabase-traffic"));
      const trafficColumns = ["DEV", "PROD"].map((environment) => ({ environment, item: trafficByEnvironment(environment) }));
      const trafficBadgeFor = (item) => healthBadge(item?.state, { ready: "LIVE", missing: item?.state === "NOT_CONNECTED" ? (item?.projectRef ? "NINCS TOKEN" : "NINCS REF") : "ELLENŐRIZD" });
      const trafficState = trafficColumns.some(({ item }) => item?.state === "DEGRADED") ? "DEGRADED" : trafficColumns.every(({ item }) => item?.state === "READY") ? "READY" : "NOT_CONNECTED";
      const trafficHeader = trafficColumns.map(({ environment, item }) => `<th scope="col"><div class="health-table__server-head"><strong title="${escapeHtml(item?.projectName || environment)}">${environment}</strong>${trafficBadgeFor(item)}</div></th>`).join("");
      const trafficRows = [["API","apiRequests"],["REST","restRequests"],["Auth","authRequests"],["Storage","storageRequests"],["Realtime","realtimeRequests"],["Egress","egressBytes"],["Cached","cachedEgressBytes"]];
      const trafficValue = (item, key) => key === "egressBytes" || key === "cachedEgressBytes" ? (item?.[key] == null ? "—" : formatBytes(item[key])) : formatCount(item?.[key]);
      const supabaseRows = trafficRows.map(([label, key]) => `<tr><th scope="row">${label}</th>${trafficColumns.map(({ item }) => `<td>${escapeHtml(trafficValue(item, key))}</td>`).join("")}</tr>`).join("");
      const supabaseSetup = '<button type="button" class="health-inline-action" data-health-action="open-supabase-monitoring">BEÁLLÍTÁS</button>';
      const supabaseNote = trafficColumns.map(({ environment, item }) => `${environment}: ${item?.reason || "Nincs projektadat."}`).join(" · ");
      const supabaseTable = `<section class="health-group health-group--supabase" data-health-state="${escapeHtml(trafficState)}"><div class="health-group__head"><h4>SUPABASE</h4><div class="health-group__actions">${supabaseSetup}</div></div><div class="health-table-wrap"><table class="health-table health-table--supabase"><thead><tr><th scope="col" class="health-table__metric">MUTATÓ</th>${trafficHeader}</tr></thead><tbody>${supabaseRows}</tbody></table><p class="health-group__note" title="${escapeHtml(supabaseNote)}">${escapeHtml(supabaseNote)}</p></div></section>`;
