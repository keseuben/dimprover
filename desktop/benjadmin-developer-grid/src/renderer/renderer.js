...
const WORKER_SURFACE_LABELS = Object.freeze({ CHATGPT:"ChatGPT", CODEX:"Codex", WORK:"Work · v0.1.50" });
...
function renderConfig() {
  ...
  for (const cellConfig of state.config.cells) {
    ...
    if (surfaceSelect) {
      ...
      surfaceSelect.title = locked ? "Aktív task közben a worker surface nem váltható." : surfaceType === "CODEX" ? "Codex · OpenAI first-party Task Bridge" : surfaceType === "WORK" ? "Work · v0.1.50-ra előkészítve · visszaváltható" : "Worker felület kiválasztása";
    }
    ...
    if (emptyState) {
      ...
      if (surfaceType === "CODEX") {
        ...
        if (strong) strong.textContent = surfaceType === "WORK" ? "Work · OpenAI first-party surface · v0.1.50 adapter." : "A ChatGPT felület zárva.";
