...
(() => {
  ...
  function selectedWorkerSurface(){const code=workerUiCode(state.workStartWorkerCode);const cell=(state.config?.cells||[]).find(c=>String(c.workerCode||"").toUpperCase()===code);return String(cell?.surfaceType||"CHATGPT").toUpperCase();}
  ...
  function workStartView() {
    ...
    const launchOptions=isCodex
      ...
      : isWork
        ? `<fieldset class="cw-chat-mode"><legend>ChatGPT Work</legend><label class="is-selected"><span><strong>OPENAI FIRST-PARTY · WORK</strong><small>A Work adapter v0.1.65-ben aktiválódik. A v0.1.61 patchben taskindítás továbbra is tiltott.</small></span></label></fieldset>`
  ...
  async function startWork(){
    ...
    const surfaceType=selectedWorkerSurface();
    if(surfaceType==="WORK"){state.workStartStatus="BLOKKOLT";state.workStartNotice="A Work surface OpenAI first-party, de a Work adapter v0.1.56-ban aktiválódik. Task nem jött létre.";state.workStartNoticeTone="warning";render();return;}
    ...
    if(result.work?.routingState==="WAITING_FOR_WORKER"){
      const refreshed=await api.getDeveloperGridActiveWork?.();
      if(refreshed?.ok&&refreshed.activeWork)state.activeWork=refreshed.activeWork;
      state.workStartStatus="VÁRAKOZIK";
      const preserved=result.work?.preservedActiveTaskId?` Az aktív ${result.work.preservedActiveTaskId} task authoritative maradt.`:"";
      state.workStartNotice=`${state.workStartWorkerCode} jelenleg nem fogadhatja az új taskot. A task várólistán marad; automatikus fallback tiltva.${preserved}`;
  ...
  async function refresh(showLoading=true) {
    ...
    if(taskId){
      ...
      const [evidenceResult,reviewResult,buildGateResult,closureResult,memoryResult]=await Promise.all([api.getDeveloperGridEvidence?.(taskId),api.getDeveloperGridReviewGate?.(taskId,"REVIEW"),api.getDeveloperGridReviewGate?.(taskId,"BUILD"),api.getDeveloperGridReviewGate?.(taskId,"CLOSURE"),activeSession?api.getDeveloperGridConversationMemory?.({taskId,sessionId:activeSession.id,conversationId:activeSession.developmentContext?.surfaceConversationId||activeSession.developmentContext?.chatConversationId||""}):null]);
  ...
  api.onContextLayout?.(applyLayout);api.onContextRefresh?.(()=>void refresh(false));api.onConfig?.(cfg=>{state.config=cfg||state.config;render();});api.onHandoffState?.(({workerCode,record})=>updateHandoffButton(workerCode,record));api.onSecurityState?.(sec=>{if(toolbarButton)toolbarButton.disabled=sec?.unlocked!==true;if(sec?.unlocked){void refresh(false);void initHandoffState();startHandoffPolling();startBuildPolling();}}); if(!detached){bindWorkerDrops();bindHandoffButtons();} Promise.all([api.getConfig?.(),api.getSecurityState?.()]).then(([cfg,sec])=>{if(toolbarButton)toolbarButton.disabled=sec?.unlocked!==true;if(cfg?.ok){state.config=cfg.config||null;document.documentElement.dataset.theme=cfg.config?.appearance==="light"?"light":"dark";applyLayout({...cfg.config?.contextWorkspace,docked:cfg.config?.contextWorkspace?.detached!==true});}if(sec?.unlocked){void refresh(false);void initHandoffState();startHandoffPolling();startBuildPolling();}else{state.notice="A Vezérlőpult zárolva van. Oldd fel a Developer Gridet a használathoz.";render();}});
})();
