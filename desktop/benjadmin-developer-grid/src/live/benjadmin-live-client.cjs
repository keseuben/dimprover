...
function synthesizeSessionTask({ session, primaryTask, stateUpdatedAt, generatedAt }) {
  ...
  return {
    ...
    mainModule: context.mainModule || null,
    moduleName: context.moduleName || null,
    submoduleName: context.submoduleName || null,
    workItem: context.workItem || null,
    workStageIndex: Number(context.workStageIndex) || null,
    sourcePrompt: context.sourcePrompt || null,
    sourceRepository: session?.sourceProvenance?.repository || null,
    sourceState: session?.sourceProvenance?.sourceState || null,
    sourceBlockCode: session?.sourceProvenance?.blockCode || null,
    sourceVerifiedAt: session?.sourceProvenance?.verifiedAt || null,
    sourceProofState: context.sourceExecutionProof?.state || null,
    sourceProofAuthority: context.sourceExecutionProof?.authority || null,
    sourceProofSha256: context.sourceExecutionProof?.sha256 || null,
    sourceProofHandshakeStage: context.sourceExecutionProof?.handshakeStage || null,
    sourceProofActiveScopeLockCount: Number(context.sourceExecutionProof?.activeScopeLockCount) || 0,
    sourceProofActiveWorktreeLeaseCount: Number(context.sourceExecutionProof?.activeWorktreeLeaseCount) || 0,
    sourceProofProductionAccess: context.sourceExecutionProof?.productionAccess || "DENY",
    sourceProofVerifiedAt: context.sourceExecutionProof?.verifiedAt || null,
...
function synthesizeGridSnapshot({ foundation, state, liveEventsByWorker, generatedAt, transport = "GRID_DELTA_NATIVE" }) {
  ...
  const workerPresence = activeSessions.map((session) => {
    ...
    return {
      ...
      nextStep: context.contextSnapshotSummary ? String(context.contextSnapshotSummary).split("\n").find((line) => line.startsWith("Következő fázis:")) || null : null,
      rawTranscriptState: context.rawTranscriptState || null,
      rawTranscriptCapturedAt: context.rawTranscriptCapturedAt || null,
      contextSnapshotId: context.contextSnapshotId || null,
      contextRevision: Number(context.contextRevision) || null,
...
function sanitizeSnapshot(live) {
  ...
  const tasks = Array.isArray(live?.tasks) ? live.tasks.map((item) => ({
    ...
    mainModule: item?.mainModule ? String(item.mainModule).slice(0,180) : (item?.metadata?.mainModule ? String(item.metadata.mainModule).slice(0,180) : null),
    moduleName: item?.moduleName ? String(item.moduleName).slice(0,180) : (item?.metadata?.moduleName ? String(item.metadata.moduleName).slice(0,180) : null),
    submoduleName: item?.submoduleName ? String(item.submoduleName).slice(0,180) : (item?.metadata?.submoduleName ? String(item.metadata.submoduleName).slice(0,180) : null),
    workItem: item?.workItem ? String(item.workItem).slice(0,1200) : (item?.metadata?.workItem ? String(item.metadata.workItem).slice(0,1200) : null),
    workStageIndex: Number(item?.workStageIndex || item?.metadata?.workStageIndex) || null,
    sourcePrompt: item?.sourcePrompt ? String(item.sourcePrompt).slice(0,7000) : (item?.metadata?.sourcePrompt ? String(item.metadata.sourcePrompt).slice(0,7000) : null),
    sourceRepository: item?.sourceRepository ? String(item.sourceRepository).slice(0,900) : (item?.metadata?.sourceRepository ? String(item.metadata.sourceRepository).slice(0,900) : null),
    sourceState: item?.sourceState ? String(item.sourceState).slice(0,30) : (item?.metadata?.sourceState ? String(item.metadata.sourceState).slice(0,30) : null),
    sourceBlockCode: item?.sourceBlockCode ? String(item.sourceBlockCode).slice(0,100) : (item?.metadata?.sourceBlockCode ? String(item.metadata.sourceBlockCode).slice(0,100) : null),
    sourceVerifiedAt: item?.sourceVerifiedAt ? String(item.sourceVerifiedAt) : (item?.metadata?.sourceVerifiedAt ? String(item.metadata.sourceVerifiedAt) : null),
    sourceProofState: item?.sourceProofState ? String(item.sourceProofState).slice(0,30) : (item?.metadata?.sourceProofState ? String(item.metadata.sourceProofState).slice(0,30) : null),
    sourceProofAuthority: item?.sourceProofAuthority ? String(item.sourceProofAuthority).slice(0,40) : (item?.metadata?.sourceProofAuthority ? String(item.metadata.sourceProofAuthority).slice(0,40) : null),
    sourceProofSha256: item?.sourceProofSha256 ? String(item.sourceProofSha256).slice(0,64) : (item?.metadata?.sourceProofSha256 ? String(item.metadata.sourceProofSha256).slice(0,64) : null),
    sourceProofHandshakeStage: item?.sourceProofHandshakeStage ? String(item.sourceProofHandshakeStage).slice(0,30) : (item?.metadata?.sourceProofHandshakeStage ? String(item.metadata.sourceProofHandshakeStage).slice(0,30) : null),
    sourceProofActiveScopeLockCount: Number(item?.sourceProofActiveScopeLockCount || item?.metadata?.sourceProofActiveScopeLockCount) || 0,
    sourceProofActiveWorktreeLeaseCount: Number(item?.sourceProofActiveWorktreeLeaseCount || item?.metadata?.sourceProofActiveWorktreeLeaseCount) || 0,
    sourceProofProductionAccess: "DENY",
    sourceProofVerifiedAt: item?.sourceProofVerifiedAt ? String(item.sourceProofVerifiedAt) : (item?.metadata?.sourceProofVerifiedAt ? String(item.metadata.sourceProofVerifiedAt) : null),
    continuityPreviousTaskId: item?.continuityPreviousTaskId ? String(item.continuityPreviousTaskId) : (item?.metadata?.continuityPreviousTaskId ? String(item.metadata.continuityPreviousTaskId) : null),
    ...
    continuityHandoffId: item?.continuityHandoffId ? String(item.continuityHandoffId) : (item?.metadata?.continuityHandoffId ? String(item.metadata.continuityHandoffId) : null),
    continuityHandoffSummary: item?.continuityHandoffSummary ? String(item.continuityHandoffSummary).slice(0,1200) : (item?.metadata?.continuityHandoffSummary ? String(item.metadata.continuityHandoffSummary).slice(0,1200) : null),
    continuityContextSnapshotId: item?.continuityContextSnapshotId ? String(item.continuityContextSnapshotId).slice(0,240) : (item?.metadata?.continuityContextSnapshotId ? String(item.metadata.continuityContextSnapshotId).slice(0,240) : null),
    continuityContextRevision: Number(item?.continuityContextRevision || item?.metadata?.continuityContextRevision) || null,
    continuityContextSummary: item?.continuityContextSummary ? String(item.continuityContextSummary).slice(0,5000) : (item?.metadata?.continuityContextSummary ? String(item.metadata.continuityContextSummary).slice(0,5000) : null),
    continuityRouting: item?.continuityRouting ? String(item.continuityRouting) : (item?.metadata?.continuityRouting ? String(item.metadata.continuityRouting) : null),
    rawTranscriptState: item?.rawTranscriptState ? String(item.rawTranscriptState).slice(0,30) : (item?.metadata?.rawTranscriptState ? String(item.metadata.rawTranscriptState).slice(0,30) : null),
    rawTranscriptCapturedAt: item?.rawTranscriptCapturedAt ? String(item.rawTranscriptCapturedAt) : (item?.metadata?.rawTranscriptCapturedAt ? String(item.metadata.rawTranscriptCapturedAt) : null),
    contextSnapshotId: item?.contextSnapshotId ? String(item.contextSnapshotId).slice(0,240) : (item?.metadata?.contextSnapshotId ? String(item.metadata.contextSnapshotId).slice(0,240) : null),
    contextRevision: Number(item?.contextRevision || item?.metadata?.contextRevision) || null,
    contextSnapshotSummary: item?.contextSnapshotSummary ? String(item.contextSnapshotSummary).slice(0,5000) : (item?.metadata?.contextSnapshotSummary ? String(item.metadata.contextSnapshotSummary).slice(0,5000) : null),
    handoffPackId: item?.handoffPackId ? String(item.handoffPackId).slice(0,240) : (item?.metadata?.handoffPackId ? String(item.metadata.handoffPackId).slice(0,240) : null),
    handoffPackState: item?.handoffPackState ? String(item.handoffPackState).slice(0,30) : (item?.metadata?.handoffPackState ? String(item.metadata.handoffPackState).slice(0,30) : null),
  ...
  const normalizePresence = (item) => ({
    ...
    rawTranscriptState: item?.rawTranscriptState ? String(item.rawTranscriptState).slice(0,30) : null,
    rawTranscriptCapturedAt: item?.rawTranscriptCapturedAt ? String(item.rawTranscriptCapturedAt) : null,
    contextSnapshotId: item?.contextSnapshotId ? String(item.contextSnapshotId).slice(0,240) : null,
    contextRevision: Number(item?.contextRevision) || null,
    contextSnapshotSummary: item?.contextSnapshotSummary ? String(item.contextSnapshotSummary).slice(0,5000) : null,
    handoffPackState: item?.handoffPackState ? String(item.handoffPackState).slice(0,30) : null,
    handoffPackId: item?.handoffPackId ? String(item.handoffPackId).slice(0,240) : null,
