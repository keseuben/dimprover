-- DIMPRO Commerce schema conformance v0.1.9 — DEV migration
-- Hard rules: monetary NUMERIC(19,4), stock/order quantities NUMERIC(19,6), UTC timestamptz.
-- Historical *_minor bigint columns stored whole monetary units in the current pilot and are renamed without /100 conversion.

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM public.commerce_external_inventory_snapshots WHERE abs(quantity) >= 10000000000000::numeric)
    OR EXISTS (SELECT 1 FROM public.commerce_goods_receipt_items WHERE abs(quantity) >= 10000000000000::numeric)
    OR EXISTS (SELECT 1 FROM public.commerce_inventory_balances WHERE abs(physical_quantity) >= 10000000000000::numeric OR abs(reserved_quantity) >= 10000000000000::numeric OR abs(incoming_quantity) >= 10000000000000::numeric)
    OR EXISTS (SELECT 1 FROM public.commerce_inventory_reservations WHERE abs(requested_quantity) >= 10000000000000::numeric OR abs(released_quantity) >= 10000000000000::numeric OR abs(consumed_quantity) >= 10000000000000::numeric)
    OR EXISTS (SELECT 1 FROM public.commerce_inventory_reservation_events WHERE abs(quantity) >= 10000000000000::numeric)
    OR EXISTS (SELECT 1 FROM public.commerce_order_items WHERE abs(quantity) >= 10000000000000::numeric)
    OR EXISTS (SELECT 1 FROM public.commerce_stock_movements WHERE abs(physical_delta) >= 10000000000000::numeric OR abs(reserved_delta) >= 10000000000000::numeric OR abs(incoming_delta) >= 10000000000000::numeric)
  THEN RAISE EXCEPTION 'COMMERCE_SCHEMA_QUANTITY_NUMERIC_19_6_OVERFLOW'; END IF;

  IF EXISTS (SELECT 1 FROM public.commerce_prices WHERE abs(amount_minor) >= 1000000000000000::numeric)
    OR EXISTS (SELECT 1 FROM public.commerce_goods_receipt_items WHERE unit_cost_minor IS NOT NULL AND abs(unit_cost_minor) >= 1000000000000000::numeric)
    OR EXISTS (SELECT 1 FROM public.commerce_order_items WHERE abs(price_net_minor) >= 1000000000000000::numeric)
  THEN RAISE EXCEPTION 'COMMERCE_SCHEMA_MONEY_NUMERIC_19_4_OVERFLOW'; END IF;
END $$;

-- Quantity conformance.
alter table public.commerce_external_inventory_snapshots
...
alter table public.commerce_stock_movements
  alter column physical_delta type numeric(19,6) using physical_delta::numeric(19,6),
  alter column reserved_delta type numeric(19,6) using reserved_delta::numeric(19,6),
  alter column incoming_delta type numeric(19,6) using incoming_delta::numeric(19,6);
...
alter table public.commerce_inventory_balances
  alter column physical_quantity type numeric(19,6) using physical_quantity::numeric(19,6),
  alter column reserved_quantity type numeric(19,6) using reserved_quantity::numeric(19,6),
  alter column incoming_quantity type numeric(19,6) using incoming_quantity::numeric(19,6);
alter table public.commerce_inventory_balances
  add column available_quantity numeric(19,6) generated always as (physical_quantity - reserved_quantity) stored;
comment on column public.commerce_inventory_balances.available_quantity is 'Generated read model: physical_quantity - reserved_quantity. NUMERIC(19,6).';
...
alter table public.commerce_inventory_reservations
  alter column requested_quantity type numeric(19,6) using requested_quantity::numeric(19,6),
  alter column released_quantity type numeric(19,6) using released_quantity::numeric(19,6),
  alter column consumed_quantity type numeric(19,6) using consumed_quantity::numeric(19,6);
alter table public.commerce_inventory_reservations
  add column remaining_quantity numeric(19,6) generated always as (requested_quantity - released_quantity - consumed_quantity) stored;

-- Monetary conformance. Existing values are whole monetary units despite legacy *_minor naming.
alter table public.commerce_prices rename column amount_minor to amount;
alter table public.commerce_prices alter column amount type numeric(19,4) using amount::numeric(19,4);
alter table public.commerce_goods_receipt_items rename column unit_cost_minor to unit_cost;
alter table public.commerce_goods_receipt_items alter column unit_cost type numeric(19,4) using unit_cost::numeric(19,4);
alter table public.commerce_order_items rename column price_net_minor to price_net;
alter table public.commerce_order_items alter column price_net type numeric(19,4) using price_net::numeric(19,4);

-- Inventory ledger: local arithmetic follows NUMERIC(19,6).
...
-- Reservation action local arithmetic follows NUMERIC(19,6).
...
begin
  ...
  select * into v_event from public.commerce_inventory_reservation_events where organization_id=p_organization_id and idempotency_key=btrim(p_idempotency_key);
  if found then
    if v_event.reservation_id<>p_reservation_id or v_event.action<>v_action or v_event.quantity<>p_quantity then raise exception 'COMMERCE_RESERVATION_IDEMPOTENCY_PAYLOAD_MISMATCH'; end if;
  ...
  select * into v_res from public.commerce_inventory_reservations where organization_id=p_organization_id and id=p_reservation_id and archived_at is null for update;
  ...
  if v_action='RELEASE' then v_reserved_delta := -p_quantity; v_movement_type := 'RESERVATION_RELEASE';
  else v_physical_delta := -p_quantity; v_reserved_delta := -p_quantity; v_movement_type := 'SALE'; end if;
  ...
  if v_action='RELEASE' then update public.commerce_inventory_reservations set released_quantity=released_quantity+p_quantity where id=p_reservation_id and organization_id=p_organization_id;
  else update public.commerce_inventory_reservations set consumed_quantity=consumed_quantity+p_quantity where id=p_reservation_id and organization_id=p_organization_id; end if;
  ...
  if v_remaining=0 then if v_res.consumed_quantity>0 then v_status:='CONSUMED'; else v_status:='RELEASED'; end if;
...
-- Receiving totals follow NUMERIC(19,6). Monetary unit_cost is not altered during stock posting.
...
begin
  ...
  select * into v_receipt from public.commerce_goods_receipts where id=p_receipt_id and organization_id=p_organization_id and archived_at is null for update;
  if not found then raise exception 'COMMERCE_RECEIPT_NOT_FOUND'; end if;
  if v_receipt.status='POSTED' then
    if v_receipt.post_idempotency_key=btrim(p_idempotency_key) then return jsonb_build_object('duplicate',true,'receiptId',v_receipt.id,'status',v_receipt.status,'postedAt',v_receipt.posted_at); end if;
  ...
  select * into v_source from public.commerce_inventory_sources where id=v_receipt.source_id and organization_id=p_organization_id and source_type='INTERNAL' and active and archived_at is null;
  ...
  for v_item in select * from public.commerce_goods_receipt_items where organization_id=p_organization_id and receipt_id=p_receipt_id and archived_at is null order by created_at,id loop
    if not exists (select 1 from public.commerce_product_variants v where v.id=v_item.variant_id and v.organization_id=p_organization_id and v.archived_at is null and v.status<>'ARCHIVED') then raise exception 'COMMERCE_RECEIPT_VARIANT_SCOPE_MISMATCH'; end if;
    v_movement := public.commerce_inventory_apply_movement(
      p_organization_id,v_receipt.source_id,v_item.variant_id,v_item.stock_status,'RECEIPT',v_item.quantity,0,0,
      'goods-receipt:post:'||p_receipt_id::text||':item:'||v_item.id::text,'GOODS_RECEIPT_ITEM',v_item.id,coalesce(p_posted_at,now())
    );
    v_items := v_items + 1; v_total := v_total + v_item.quantity;
  end loop;
  if v_items=0 then raise exception 'COMMERCE_RECEIPT_EMPTY'; end if;
  update public.commerce_goods_receipts set status='POSTED',posted_at=coalesce(p_posted_at,now()),post_idempotency_key=btrim(p_idempotency_key) where id=p_receipt_id and organization_id=p_organization_id;
  insert into public.commerce_audit_events(organization_id,action,entity_type,entity_id,metadata)
  values (p_organization_id,'GOODS_RECEIPT_POSTED','GOODS_RECEIPT',p_receipt_id,jsonb_build_object('itemCount',v_items,'totalQuantity',v_total,'warehouseId',v_receipt.warehouse_id,'sourceId',v_receipt.source_id));
  insert into public.commerce_outbox_events(organization_id,aggregate_type,aggregate_id,event_type,payload,idempotency_key)
  values (p_organization_id,'GOODS_RECEIPT',p_receipt_id,'GOODS_RECEIPT_POSTED',jsonb_build_object('receiptId',p_receipt_id,'itemCount',v_items,'totalQuantity',v_total),'goods-receipt-posted:'||p_receipt_id::text);
...
-- Canonical monetary price setter: NUMERIC(19,4), no bigint/minor-unit storage.
...
begin
  if p_organization_id is null or not exists (select 1 from public.dimpro_organizations o where o.id=p_organization_id and o.status='active') then raise exception 'COMMERCE_ORGANIZATION_NOT_ACTIVE'; end if;
  if not exists (select 1 from public.commerce_product_variants v where v.id=p_variant_id and v.organization_id=p_organization_id and v.archived_at is null and v.status <> 'ARCHIVED') then raise exception 'COMMERCE_PRICE_VARIANT_SCOPE_MISMATCH'; end if;
  if v_currency not in ('HUF','EUR','USD') then raise exception 'COMMERCE_PRICE_CURRENCY_INVALID'; end if;
  if p_amount is null or p_amount < 0 or p_amount >= 1000000000000000::numeric or round(p_amount,4)<>p_amount then raise exception 'COMMERCE_PRICE_AMOUNT_INVALID'; end if;
  v_amount := p_amount::numeric(19,4);
  if p_vat_rate_basis_points is null or p_vat_rate_basis_points < 0 or p_vat_rate_basis_points > 10000 then raise exception 'COMMERCE_PRICE_VAT_INVALID'; end if;
  perform pg_advisory_xact_lock(hashtextextended(concat_ws('|',p_organization_id::text,p_variant_id::text,v_currency),0));
  update public.commerce_prices set status='INACTIVE',valid_until=case when valid_until is null or valid_until > v_effective then v_effective else valid_until end,updated_at=now()
  where organization_id=p_organization_id and variant_id=p_variant_id and currency=v_currency and status='ACTIVE' and archived_at is null and (valid_until is null or valid_until > v_effective);
  get diagnostics v_previous_count = row_count;
  insert into public.commerce_prices(organization_id,variant_id,currency,amount,vat_rate_basis_points,valid_from,valid_until,status)
  values (p_organization_id,p_variant_id,v_currency,v_amount,p_vat_rate_basis_points,v_effective,null,'ACTIVE') returning id into v_price_id;
  insert into public.commerce_audit_events(organization_id,action,entity_type,entity_id,metadata)
  values (p_organization_id,'PRICE_SET_ACTIVE','PRICE',v_price_id,jsonb_build_object('variantId',p_variant_id,'currency',v_currency,'amount',v_amount,'vatRateBasisPoints',p_vat_rate_basis_points,'previousDeactivated',v_previous_count));
  insert into public.commerce_outbox_events(organization_id,aggregate_type,aggregate_id,event_type,payload,idempotency_key)
  values (p_organization_id,'PRODUCT_VARIANT',p_variant_id,'PRICE_CHANGED',jsonb_build_object('priceId',v_price_id,'variantId',p_variant_id,'currency',v_currency,'amount',v_amount,'vatRateBasisPoints',p_vat_rate_basis_points),'price-changed:'||v_price_id::text);
  return jsonb_build_object('priceId',v_price_id,'variantId',p_variant_id,'currency',v_currency,'amount',v_amount,'vatRateBasisPoints',p_vat_rate_basis_points,'effectiveAt',v_effective,'previousDeactivated',v_previous_count);
end;
$$;
revoke all on function public.commerce_price_set_active(uuid,uuid,text,numeric,integer,timestamptz) from public,anon,authenticated;
grant execute on function public.commerce_price_set_active(uuid,uuid,text,numeric,integer,timestamptz) to service_role;

-- Shared order create keeps old JSON input alias only for one-release compatibility, but persists canonical NUMERIC fields.
create or replace function public.commerce_order_create_atomic(
  p_organization_id uuid,p_order_number text,p_source_channel text,p_external_reference text,p_customer_name text,p_customer_type text,
  p_recorder_name text,p_pickup_at timestamptz,p_note text,p_initial_status text,p_items jsonb,p_created_by_user_id uuid,p_idempotency_key text
) returns jsonb
language plpgsql
security definer
set search_path=public
as $$
declare
  v_order_id uuid; v_existing public.commerce_orders%rowtype; v_item jsonb; v_product_id uuid; v_variant_id uuid;
  v_product_name text; v_sku text; v_unit text; v_quantity numeric(19,6); v_price numeric(19,4); v_vat integer; v_storage_zone text;
  v_count integer:=0; v_payload_hash text; v_status text:=upper(coalesce(p_initial_status,'SENT_TO_CASHIER'));
  v_source text:=upper(coalesce(p_source_channel,'')); v_customer_type text:=upper(coalesce(p_customer_type,''));
begin
  if p_organization_id is null or not exists(select 1 from public.dimpro_organizations o where o.id=p_organization_id and o.status='active') then raise exception 'COMMERCE_ORGANIZATION_NOT_ACTIVE'; end if;
  ...
  v_payload_hash:=md5(jsonb_build_object('orderNumber',btrim(p_order_number),'sourceChannel',v_source,'externalReference',nullif(btrim(p_external_reference),''),'customerName',btrim(p_customer_name),'customerType',v_customer_type,'recorderName',nullif(btrim(p_recorder_name),''),'pickupAt',p_pickup_at,'note',nullif(btrim(p_note),''),'initialStatus',v_status,'items',p_items)::text);
  ...
  if found then if v_existing.create_payload_hash<>v_payload_hash then raise exception 'COMMERCE_ORDER_IDEMPOTENCY_PAYLOAD_MISMATCH'; end if; return jsonb_build_object('duplicate',true,'orderId',v_existing.id,'orderNumber',v_existing.order_number,'status',v_existing.status); end if;
  ...
  for v_item in select value from jsonb_array_elements(p_items) loop
    ...
    v_product_name:=btrim(coalesce(v_item->>'productName','')); v_sku:=nullif(btrim(coalesce(v_item->>'sku','')),''); v_unit:=upper(btrim(coalesce(v_item->>'unit','')));
    begin v_quantity:=coalesce(nullif(v_item->>'quantity','')::numeric,0)::numeric(19,6); exception when others then raise exception 'COMMERCE_ORDER_QUANTITY_INVALID'; end;
    begin v_price:=coalesce(nullif(coalesce(v_item->>'priceNet',v_item->>'priceNetMinor'),'')::numeric,-1)::numeric(19,4); exception when others then raise exception 'COMMERCE_ORDER_PRICE_INVALID'; end;
    ...
    if v_quantity<=0 then raise exception 'COMMERCE_ORDER_QUANTITY_INVALID'; end if; if v_price<0 then raise exception 'COMMERCE_ORDER_PRICE_INVALID'; end if; if v_vat<0 or v_vat>10000 then raise exception 'COMMERCE_ORDER_VAT_INVALID'; end if;
    ...
    insert into public.commerce_order_items(organization_id,order_id,product_id,variant_id,inventory_status,product_name,sku,unit,quantity,price_net,vat_rate_basis_points,storage_zone)
