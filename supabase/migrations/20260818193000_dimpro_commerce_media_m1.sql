-- DIMPRO Commerce Media Engine M1 v0.1.1 — DEV migration
-- Add immutable media variants + non-destructive overlays + atomic metadata finalization.

create table if not exists public.commerce_media_variants (
  ...
  variant_kind text not null check (variant_kind in ('ORIGINAL','WEB','THUMBNAIL')),
  ...
  unique (organization_id, asset_id, variant_kind),
  unique (organization_id, storage_key)
);

create table if not exists public.commerce_media_overlays (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null references public.dimpro_organizations(id) on delete restrict,
  asset_id uuid not null references public.commerce_media_assets(id) on delete cascade,
  overlay_type text not null check (overlay_type in ('WATERMARK','LOGO','STAMP','ARROW','CIRCLE','TEXT','BLUR')),
...
create index if not exists commerce_media_variants_asset_idx
  on public.commerce_media_variants (organization_id, asset_id, variant_kind)
  where archived_at is null;
create index if not exists commerce_media_overlays_asset_idx
  on public.commerce_media_overlays (organization_id, asset_id, sort_order)
  where archived_at is null and active;

drop trigger if exists commerce_media_variants_updated_at_trigger on public.commerce_media_variants;
create trigger commerce_media_variants_updated_at_trigger
before update on public.commerce_media_variants
for each row execute function public.dimpro_set_updated_at();

drop trigger if exists commerce_media_overlays_updated_at_trigger on public.commerce_media_overlays;
create trigger commerce_media_overlays_updated_at_trigger
before update on public.commerce_media_overlays
for each row execute function public.dimpro_set_updated_at();

alter table public.commerce_media_variants enable row level security;
alter table public.commerce_media_overlays enable row level security;
revoke all on table public.commerce_media_variants, public.commerce_media_overlays from anon, authenticated, service_role;
grant select,insert,update,delete on table public.commerce_media_variants, public.commerce_media_overlays to service_role;
