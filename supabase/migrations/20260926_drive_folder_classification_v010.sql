...
begin
  update public.drive_core_folders set discipline=left(btrim(coalesce(p_discipline,)),120), topic=left(btrim(coalesce(p_topic,)),160), updated_at=now()
  where id=p_folder_id and project_id=p_project_id and status=ACTIVE returning * into v_folder;
  if v_folder.id is null then raise exception DRIVE_CORE_FOLDER_NOT_FOUND using errcode=P0002; end if;
  insert into public.drive_core_change_events(id,project_id,event_type,entity_type,entity_id,payload,actor_user_id)
  values(drive-change-||substr(replace(gen_random_uuid()::text,-,),1,16),p_project_id,FOLDER_CLASSIFICATION_UPDATED,folder,v_folder.id,to_jsonb(v_folder),p_actor_user_id);
