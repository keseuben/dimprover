...
-- Extend Commerce Media finalization to Receiving targets.
...
begin
  ...
  for v_item in select value from jsonb_array_elements(p_links) loop
    ...
    elsif v_link_type='GOODS_RECEIPT' then
      if not exists (select 1 from public.commerce_goods_receipts r where r.id=v_linked_id and r.organization_id=p_organization_id and r.archived_at is null) then
        raise exception 'COMMERCE_MEDIA_RECEIPT_SCOPE_MISMATCH';
      end if;
    elsif v_link_type='GOODS_RECEIPT_ITEM' then
      if not exists (select 1 from public.commerce_goods_receipt_items i where i.id=v_linked_id and i.organization_id=p_organization_id and i.archived_at is null) then
        raise exception 'COMMERCE_MEDIA_RECEIPT_ITEM_SCOPE_MISMATCH';
      end if;
    else
