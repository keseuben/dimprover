-- DIMPRO Commerce Storefront Mirror Queue M1 v0.1.12 — DEV migration
-- Public Storefront requests enqueue a trusted-tenant mirror job without impersonating a Commerce user.
...
begin
  ...
  values(
    ...
    'legacy-order-mirror-queued:'||v_attempt.id::text||':'||v_status||':'||v_payload_hash
...
create index commerce_order_mirror_attempts_retry_idx
  ...
  where deleted_at is null and state in ('PENDING','FAILED');

update public.commerce_schema_meta
set schema_version='0.1.12',migration_count=13,bootstrap_id='commerce-storefront-mirror-queue-m1-20260819',updated_at=now()
