-- DEV rollback: Commerce P2 pilot unit alignment.
-- Precondition: no active RAKLAP/ZSAK/LADA ProductVariant or GoodsReceiptItem records may remain.

do $$
begin
  if exists (
    select 1 from public.commerce_product_variants
    where deleted_at is null and unit in ('RAKLAP','ZSAK','LADA')
  ) then
    raise exception 'COMMERCE_P2_ROLLBACK_EXTENDED_PRODUCT_UNITS_IN_USE';
  end if;
  if exists (
    select 1 from public.commerce_goods_receipt_items
    where deleted_at is null and unit in ('RAKLAP','ZSAK','LADA')
  ) then
    raise exception 'COMMERCE_P2_ROLLBACK_EXTENDED_RECEIVING_UNITS_IN_USE';
  end if;
end;
$$;
...
alter table public.commerce_product_variants
  ...
  check (unit in ('DB','KG','G','M','M2','M3','FM','L','CSOMAG','PAR','KESZLET'));
...
set schema_version='0.1.14',
    migration_count=15,
    bootstrap_id='commerce-storefront-product-mapping-p1-20260821',
    updated_at=now()
