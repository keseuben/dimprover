...
## 2026-09-09 – Automatikus Worktree Retention V3

A DEV VPS teljes worktree-retentionje külön, fail-closed V3 motort kapott. A napi futás 04:20 Europe/Budapest időpontban történik, a Restic backup és a korábbi storage-retention után. A motor csak akkor távolít el teljes worktree-t, ha a szabad hely 30 GiB alá esett. Worktree csak Git-clean, attached branch, azonos canonical repository, processz-/PM2-/systemd-/cron-/aktív művelet mentes állapotban, legalább 14 nap inaktivitás után és legfeljebb 30 órás sikeres Restic mentés mellett lehet jelölt. A backupnak a HEAD commitnál frissebbnek kell lennie.

Családonként a legfrissebb 3 worktree 30 napig garantáltan megmarad. A 30 nap letelte után ez a védelem lejár, hogy az egyszeri vagy ritka modulfejlesztések ne halmozódjanak korlátlanul. Törlés előtt archive ref és 0600 recovery manifest készül; az eltávolítás kizárólag `git worktree remove` művelettel történik, `--force` és `rm -rf` nélkül. Bizonytalan állapotban a motor nem töröl. Részletes szabály: `263_dimpro_dev_worktree_retention_v3_20260909.md`.

### BENJADMIN Developer Grid v0.1.33 – Conversation Memory üzemeltetés

A Developer Grid Conversation Memory kizárólag DEV-en tárolható a `/srv/dimpro-dev/coordination/developer-grid/conversation-memory` alatt. A könyvtár 0700, a RAW/context/handoff fájlok 0600 jogosultságúak. A Fekete doboz nyers beszélgetéstartalmat tartalmazhat, ezért nem kerülhet publikus artifactba, `/var/www` letöltési mappába, klienslogba vagy külső AI provider promptjába. A következő tasknak csak sanitizált Context Snapshot és handoff adható át. A RAW tárolás append-only delta, ezért azonos csevegés teljes előzménye nem ismétlődik minden 8 másodperces ellenőrzésnél.

A jelenlegi általános backup/artifact retention nem törli automatikusan a Conversation Memoryt. Kézi törlés csak külön, dokumentált retention döntéssel és lezárt task/handoff visszaállíthatóságának ellenőrzésével végezhető. PROD hozzáférés, PROD mentés és PROD transcript capture tiltott.
